use super::Var;
use crate::nn::graph::RecurrentFoldingMeta;
use crate::nn::nodes::NodeInner;
use crate::nn::{GraphError, NodeId, SnapshotNode, VisualizationOutput, VisualizationSnapshot};
use std::rc::Rc;
...
impl Var {
    ...
    pub(crate) fn build_snapshot(named_vars: &[(&str, &Self)]) -> VisualizationSnapshot {
        ...
        // 提取单个节点信息的闭包
        let extract_node = |node: &Rc<NodeInner>| -> SnapshotNode {
    ...
    pub(crate) fn snapshot_to_dot(
        ...
        folding_metas: &[RecurrentFoldingMeta],
    ) -> String {
        ...
        // 快照节点 ID 查找表
        let node_map: HashMap<u64, &SnapshotNode> =
        ...
        // 节点定义生成闭包
        let generate_node_def = |snode: &SnapshotNode| -> String {
            let id = snode.id.0;
    ...
    fn save_visualization_for_vars<P: AsRef<std::path::Path>>(
        ...
        if graphviz_available {
            ...
            match output {
                Ok(o) if o.status.success() => Ok(VisualizationOutput {
                    dot_path,
