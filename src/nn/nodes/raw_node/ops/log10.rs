/*
 * @Author       : 老董
 * @Description  : Log10（以 10 为底的对数）节点
 *                 实现逐元素 log10: y = log10(x)
 */
...
impl TraitNode for Log10 {
    ...
    fn calc_value_by_parents(&mut self, parent_values: &[&Tensor]) -> Result<(), GraphError> {
        ...
        self.value = Some(parent_values[0].log10());
        Ok(())
    }
    ...
    fn calc_grad_to_parent(
        ...
        let input = self.input_cache.as_ref().ok_or_else(|| {
            GraphError::ComputationError("Log10 输入缓存为空".to_string())
        })?;
