 ...
 *   VJP: grad_to_parent = upstream_grad ⊙ gelu'(x)
 *   注意：必须从 parent_values 计算梯度（无法从 value 反推）
 *
...
/// 测试 GELU 前向传播（PyTorch tanh approximate 对照值）
///
/// gelu([0.5, -1.0, 0.0, 2.0]) ≈ [0.3457, -0.1588, 0.0, 1.9546]
#[test]
...
/// 测试 GELU 前向传播（边界值）
#[test]
fn test_gelu_forward_edge_cases() {
    let graph = Graph::new();
    let x = graph
    ...
    let output = result.value().unwrap().unwrap();
    assert_abs_diff_eq!(output[[0, 0]], 0.0, epsilon = 1e-6); // gelu(0) = 0
    assert_abs_diff_eq!(output[[0, 1]], 10.0, epsilon = 1e-4); // gelu(10) ≈ 10
    assert_abs_diff_eq!(output[[0, 2]], 0.0, epsilon = 1e-4); // gelu(-10) ≈ 0
}

/// 测试 GELU 节点不能直接设置值
#[test]
fn test_gelu_cannot_set_value() {
    let graph = Graph::new();
    let x = graph
    ...
    assert!(err.is_err(), "GELU 节点不应支持直接设值");
}

// ==================== VJP 单元测试 ====================

/// 测试 GELU VJP（全 1 上游梯度，PyTorch 对照值）
///
/// gelu'([0.5, -1.0, 0.0, 2.0]) ≈ [0.8674, -0.0830, 0.5, 1.0861]
#[test]
fn test_gelu_vjp_unit_upstream() -> Result<(), GraphError> {
    let graph = Graph::new();
    ...
    let gelu = inner
        .borrow_mut()
        .create_gelu_node(x.clone(), Some("gelu"))
        .unwrap();
    ...
    let grad = gelu.calc_grad_to_parent_index(0, &upstream_grad)?;

    assert_eq!(grad.shape(), &[2, 2]);
    assert_abs_diff_eq!(grad[[0, 0]], 0.8674, epsilon = 1e-3);
    assert_abs_diff_eq!(grad[[0, 1]], -0.0830, epsilon = 1e-3);
    assert_abs_diff_eq!(grad[[1, 0]], 0.5, epsilon = 1e-3);
    assert_abs_diff_eq!(grad[[1, 1]], 1.0861, epsilon = 1e-3);
...
/// 测试 GELU VJP（非单位上游梯度）
#[test]
fn test_gelu_vjp_non_unit_upstream() -> Result<(), GraphError> {
    let graph = Graph::new();
    let inner = graph.inner_rc();
    ...
    assert_abs_diff_eq!(grad[[0, 1]], 3.0 * (-0.0830), epsilon = 1e-2);
    assert_abs_diff_eq!(grad[[1, 0]], 4.0 * 0.5, epsilon = 1e-2);
    assert_abs_diff_eq!(grad[[1, 1]], 5.0 * 1.0861, epsilon = 1e-2);
...
/// 测试 GELU 端到端反向传播：result = gelu(x) -> loss = MSE(result, target)
#[test]
fn test_gelu_backward_e2e() -> Result<(), GraphError> {
    let graph = Graph::new();
    ...
    // gelu(0) = 0，target = 0，diff = 0，对应 grad 应为 0
...
/// 测试 GELU 梯度累积 + zero_grad
#[test]
fn test_gelu_gradient_accumulation() -> Result<(), GraphError> {
    let graph = Graph::new();
...
/// 测试 GELU 节点的动态形状传播
#[test]
fn test_gelu_dynamic_shape_propagation() {
    let graph = Graph::new();
...
/// 测试 GELU 节点在不同 batch_size 下的前向计算
#[test]
fn test_gelu_dynamic_batch_forward() {
    use crate::nn::var::ops::VarActivationOps;
...
/// 测试 GELU 节点在不同 batch_size 下的反向传播
#[test]
fn test_gelu_dynamic_batch_backward() {
...
fn test_create_gelu_node() {
    ...
    let gelu = inner
        .borrow_mut()
        .create_gelu_node(input.clone(), Some("gelu"))
        .unwrap();

    assert_eq!(gelu.shape(), vec![2, 3]);
    assert_eq!(gelu.name(), Some("gelu"));
    assert!(!gelu.is_leaf());
    assert_eq!(gelu.parents().len(), 1);
}

#[test]
fn test_create_gelu_node_preserves_shape() {
    let graph = Graph::new();
    let inner = graph.inner_rc();
...
#[test]
fn test_create_gelu_node_drop_releases() {
    let graph = Graph::new();
    let inner = graph.inner_rc();
    ...
    let weak_input;
    {
        ...
        assert!(weak_gelu.upgrade().is_some());
        assert!(weak_input.upgrade().is_some());
    }
    assert!(weak_gelu.upgrade().is_none());
