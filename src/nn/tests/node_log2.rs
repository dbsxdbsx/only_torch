/*
 * @Author       : 老董
 * @Description  : Log2（以 2 为底的对数）节点单元测试
 *
 * 梯度公式：y = log2(x), dy/dx = 1/(x * ln(2))
 *
 * Python 对照:
...
fn test_log2_forward() {
    let graph = Graph::new();
    let x = graph
        .input(&Tensor::new(&[1.0, 2.0, 4.0, 8.0], &[2, 2]))
        .unwrap();
    let result = x.log2();
...
#[test]
fn test_log2_vjp() -> Result<(), GraphError> {
    let graph = Graph::new();
    ...
    assert_abs_diff_eq!(grad[[0, 1]], 1.0 / (2.0 * ln2), epsilon = 1e-5);
    assert_abs_diff_eq!(grad[[1, 0]], 1.0 / (4.0 * ln2), epsilon = 1e-5);
    assert_abs_diff_eq!(grad[[1, 1]], 1.0 / (8.0 * ln2), epsilon = 1e-5);
    Ok(())
}

#[test]
fn test_log2_backward_e2e() -> Result<(), GraphError> {
    let graph = Graph::new();
    let x = graph.parameter(&[2, 2], Init::Zeros, "x")?;
    x.set_value(&Tensor::new(&[1.0, 2.0, 4.0, 8.0], &[2, 2]))?;
