...
fn test_huber_vjp_small_error() -> Result<(), GraphError> {
    ...
    target
        ...
        .unwrap();
    huber.forward_recursive(1, Mode::Train).unwrap();
...
fn test_huber_dynamic_batch_forward() -> Result<(), GraphError> {
    ...
    huber.forward_recursive(2, Mode::Train).unwrap();
...
/// **回归测试**：Huber 作为中间 loss 项（非终端）时梯度必须随上游梯度线性缩放
/// （历史 bug：反向忽略 `upstream_grad`）。混合 |a|≤δ 与 |a|>δ 两支，`loss * SCALE`
/// 的 input 梯度应 = 终端梯度 × SCALE。
#[test]
fn test_huber_respects_upstream_grad_when_non_terminal() -> Result<(), GraphError> {
    const SCALE: f32 = 3.0;
    // 含 |a|≤δ（-0.5）与 |a|>δ（2.0）两支，覆盖 Huber 两段梯度
    let input_data = Tensor::new(&[1.0, 2.0, 5.0], &[1, 3]);
    let target_data = Tensor::new(&[1.5, 2.5, 1.0], &[1, 3]);

    let g1 = Graph::new();
    let x1 = g1.parameter(&[1, 3], Init::Zeros, "x")?;
    x1.set_value(&input_data)?;
    let t1 = g1.input(&target_data)?;
    let loss1 = x1.huber_loss(&t1)?;
    loss1.backward()?;
    ...
    let scaled = &x2.huber_loss(&t2)? * SCALE;
    scaled.backward()?;
