 ...
 *   elu'(x) = 1 if x > 0, else elu(x) + alpha = alpha * exp(x)
 *   VJP: grad_to_parent = upstream_grad ⊙ elu'(x)
 *
 * Python 对照值（PyTorch torch.nn.functional.elu, alpha=1.0）：
 *   elu([0.5, -1.0, 0.0, 2.0]) = [0.5, -0.6321, 0.0, 2.0]
 *   elu'([0.5, -1.0, 0.0, 2.0]) = [1.0, 0.3679, 1.0, 1.0]
 *
...
/// 测试 ELU 前向传播（PyTorch 对照值，alpha=1.0）
///
/// elu([0.5, -1.0, 0.0, 2.0]) ≈ [0.5, -0.6321, 0.0, 2.0]
#[test]
fn test_elu_forward() {
    ...
    assert_abs_diff_eq!(output[[0, 1]], -0.6321, epsilon = 1e-4);
    assert_abs_diff_eq!(output[[1, 0]], 0.0, epsilon = 1e-6);
    assert_abs_diff_eq!(output[[1, 1]], 2.0, epsilon = 1e-4);
}

/// 测试 ELU 前向传播（边界值）
#[test]
fn test_elu_forward_edge_cases() {
    let graph = Graph::new();
    let x = graph
    ...
    let output = result.value().unwrap().unwrap();
    assert_abs_diff_eq!(output[[0, 0]], 0.0, epsilon = 1e-6); // elu(0) = 0
    assert_abs_diff_eq!(output[[0, 1]], 10.0, epsilon = 1e-4); // elu(10) = 10
    assert_abs_diff_eq!(output[[0, 2]], -1.0, epsilon = 1e-4); // elu(-10) ≈ -alpha = -1.0
}

/// 测试 ELU 节点不能直接设置值
#[test]
fn test_elu_cannot_set_value() {
    let graph = Graph::new();
    let x = graph
    ...
    assert!(err.is_err(), "ELU 节点不应支持直接设值");
}

// ==================== VJP 单元测试 ====================

/// 测试 ELU VJP（全 1 上游梯度，PyTorch 对照值，alpha=1.0）
///
/// elu'([0.5, -1.0, 0.0, 2.0]) ≈ [1.0, 0.3679, 1.0, 1.0]
#[test]
fn test_elu_vjp_unit_upstream() -> Result<(), GraphError> {
    let graph = Graph::new();
    ...
    let elu = inner
        .borrow_mut()
        .create_elu_node(x.clone(), 1.0, Some("elu"))
        .unwrap();
    ...
    let grad = elu.calc_grad_to_parent_index(0, &upstream_grad)?;

    assert_eq!(grad.shape(), &[2, 2]);
    assert_abs_diff_eq!(grad[[0, 0]], 1.0, epsilon = 1e-3);
    assert_abs_diff_eq!(grad[[0, 1]], 0.3679, epsilon = 1e-3);
    assert_abs_diff_eq!(grad[[1, 0]], 1.0, epsilon = 1e-3);
    assert_abs_diff_eq!(grad[[1, 1]], 1.0, epsilon = 1e-3);
...
/// 测试 ELU VJP（非单位上游梯度）
#[test]
fn test_elu_vjp_non_unit_upstream() -> Result<(), GraphError> {
    let graph = Graph::new();
    let inner = graph.inner_rc();
    ...
    assert_abs_diff_eq!(grad[[0, 1]], 3.0 * 0.3679, epsilon = 1e-2);
    assert_abs_diff_eq!(grad[[1, 0]], 4.0 * 1.0, epsilon = 1e-2);
    assert_abs_diff_eq!(grad[[1, 1]], 5.0 * 1.0, epsilon = 1e-2);
...
/// 测试 ELU 端到端反向传播：result = elu(x) -> loss = MSE(result, target)
#[test]
fn test_elu_backward_e2e() -> Result<(), GraphError> {
    let graph = Graph::new();
    ...
    // elu(0) = 0，target = 0，diff = 0，对应 grad 应为 0
...
/// 测试 ELU 梯度累积 + zero_grad
#[test]
fn test_elu_gradient_accumulation() -> Result<(), GraphError> {
    let graph = Graph::new();
...
/// 测试 ELU 节点的动态形状传播
#[test]
fn test_elu_dynamic_shape_propagation() {
    let graph = Graph::new();
...
/// 测试 ELU 节点在不同 batch_size 下的前向计算
#[test]
fn test_elu_dynamic_batch_forward() {
    use crate::nn::var::ops::VarActivationOps;
...
/// 测试 ELU 节点在不同 batch_size 下的反向传播
#[test]
fn test_elu_dynamic_batch_backward() {
...
fn test_create_elu_node() {
    ...
    let elu = inner
        .borrow_mut()
        .create_elu_node(input.clone(), 1.0, Some("elu"))
        .unwrap();

    assert_eq!(elu.shape(), vec![2, 3]);
    assert_eq!(elu.name(), Some("elu"));
    assert!(!elu.is_leaf());
    assert_eq!(elu.parents().len(), 1);
}

#[test]
fn test_create_elu_node_preserves_shape() {
    let graph = Graph::new();
    let inner = graph.inner_rc();
...
#[test]
fn test_create_elu_node_drop_releases() {
    let graph = Graph::new();
    let inner = graph.inner_rc();
    ...
    assert!(weak_elu.upgrade().is_none());
