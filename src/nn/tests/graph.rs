...
fn test_new_node_error_handling() {
    ...
    assert_eq!(
        ...
        Err(GraphError::ShapeMismatch {
            ...
            got: vec![2, 1],
            message: format!(
                "新张量的形状 [2, 1] 与节点 'variable_1' 现有张量的形状 [2, 2] 不匹配。"
            )
        })
    );
}
...
fn test_reset_graph_forward_cnt_for_forward_count_mechanism() {
    ...
    // 1.1 验证初始前向传播次数
    assert_eq!(graph.get_node(var1).unwrap().forward_cnt(), 1); // 已初始化，次数为1
    assert_eq!(graph.get_node(var2).unwrap().forward_cnt(), 0); // 未初始化，次数为0
    ...
    // 2.1 验证前向传播后的次数
    assert_eq!(graph.get_node(var1).unwrap().forward_cnt(), 1);
    assert_eq!(graph.get_node(var2).unwrap().forward_cnt(), 1);
    ...
    assert_eq!(graph.get_node(var1).unwrap().forward_cnt(), 0);
    assert_eq!(graph.get_node(var2).unwrap().forward_cnt(), 0);
    assert_eq!(graph.get_node(add).unwrap().forward_cnt(), 0);
    assert_eq!(graph.forward_cnt(), 0);

    // 4. 重置后再次前向传播
    // 4.1 由于var1和var2的前向传播次数都被重置为0，此时前向传播会失败
    assert_eq!(
        graph.forward_node(add),
        Err(GraphError::InvalidOperation(format!(
            "节点[id=1, name=var1, type=Variable]不能直接前向传播（须通过set_value或初始化时设置`init`为true来增加前向传播次数）。问题节点的前向传播次数为0，而图的前向传播次数为1",
        )))
    );
...
fn test_a_complete_case_for_forward_count_mechanism() {
    ...
    assert_eq!(
        graph.forward_node(add),
        Err(GraphError::InvalidOperation(format!(
            "节点[id=2, name=var2, type=Variable]不能直接前向传播（须通过set_value或初始化时设置`init`为true来增加前向传播次数）。问题节点的前向传播次数为0，而图的前向传播次数为1",
        )))
    );
    ...
    assert_eq!(
        graph.forward_node(add),
        Err(GraphError::InvalidOperation(format!(
            "节点[id=1, name=var1, type=Variable]不能直接前向传播（须通过set_value或初始化时设置`init`为true来增加前向传播次数）。问题节点的前向传播次数为1，而图的前向传播次数为2",
        )))
    );
    ...
    assert_eq!(
        graph.forward_node(add),
        Err(GraphError::InvalidOperation(format!(
            "节点[id=2, name=var2, type=Variable]不能直接前向传播（须通过set_value或初始化时设置`init`为true来增加前向传播次数）。问题节点的前向传播次数为1，而图的前向传播次数为2",
        )))
    );
    assert_eq!(graph.forward_cnt(), 1); // 此时图虽然执行了前向传播，但由于失败了，因此回滚后的图前向传播次数仍不变
