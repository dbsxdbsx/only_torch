...
impl GraphInner {
    ...
    const fn type_name(node_type: &NodeTypeDescriptor) -> &'static str {
        match node_type {
            ...
            NodeTypeDescriptor::Maximum => "Maximum",
            NodeTypeDescriptor::Minimum => "Minimum",
            NodeTypeDescriptor::Amax { .. } => "Amax",
            NodeTypeDescriptor::Amin { .. } => "Amin",
    ...
    pub(in crate::nn::graph) fn node_type_to_descriptor(
        ...
        match node_type {
            ...
            NodeType::Mean(node) => NodeTypeDescriptor::Mean { axis: node.axis() },
            ...
            NodeType::Amax(node) => NodeTypeDescriptor::Amax { axis: node.axis() },
            NodeType::Amin(node) => NodeTypeDescriptor::Amin { axis: node.axis() },
            ...
            NodeType::Gather(node) => NodeTypeDescriptor::Gather { dim: node.dim() },
