...
impl ForDynamicImage for DynamicImage {
    ...
    fn to_tensor(&self) -> Result<Tensor, String> {
        ...
        let mut tensor = Tensor::zeros(&[height, width, channels]);

        for y in 0..height {
            for x in 0..width {
                let pixel = self.get_pixel(x as u32, y as u32);
                tensor[[y, x, 0]] = f32::from(pixel[0]);
                if channels >= 3 {
                    tensor[[y, x, 1]] = f32::from(pixel[1]);
                    tensor[[y, x, 2]] = f32::from(pixel[2]);
                }
                if channels >= 4 {
                    tensor[[y, x, 3]] = f32::from(pixel[3]);
                }
            }
        }
        tensor.squeeze_mut();
