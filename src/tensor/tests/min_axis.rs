...
fn test_min_1d() {
    ...
    let result = x.min(0);
    // 1D 张量沿 axis=0 的 min 返回标量（0 维张量）
...
#[test]
fn test_min_consistent_with_argmin() {
    // 验证 min 和 argmin 的一致性
    let x = Tensor::new(&[5.0, 3.0, 6.0, 1.0, 4.0, 2.0], &[2, 3]);

    let min_vals = x.min(1);
...
#[test]
#[should_panic(expected = "min: axis 2 超出维度范围 2")]
fn test_min_invalid_axis() {
    let x = Tensor::new(&[1.0, 2.0, 3.0, 4.0], &[2, 2]);
    let _ = x.min(2); // 只有 axis 0 和 1 有效
}
