...
fn test_div_matrices_with_same_shape() {
    ...
    assert_eq!(
        result.data,
        Array::from_shape_vec(IxDyn(shape), vec![0.2, 1.0 / 3.0, 3.0 / 7.0, 0.5]).unwrap()
    );
}

#[test]
fn test_div_high_order_tensors_with_same_shape() {
    let shape = &[2, 1, 2];
...
fn test_div_number_and_tensor() {
    ...
    let test_cases = vec![
        ...
        TensorCheck {
            shape: vec![],
            data: vec![1.0],
            expected: vec![vec![2.0], vec![1.0 / number]],
        },
        TensorCheck {
            shape: vec![1],
            data: vec![1.0],
        ...
        TensorCheck {
            shape: vec![2],
            data: vec![1.0, 2.0],
            expected: vec![vec![2.0, 1.0], vec![1.0 / number, 1.0]],
        },
        TensorCheck {
            shape: vec![2, 1],
            data: vec![1.0, 2.0],
...
#[test]
#[should_panic(
    expected = "形状不一致且两个张量没有一个是标量，故无法相除：第一个张量的形状为[3]，第二个张量的形状为[2]"
)]
...
#[test]
#[should_panic(
    expected = "形状不一致且两个张量没有一个是标量，故无法相除：第一个张量的形状为[3]，第二个张量的形状为[3, 1]"
)]
...
fn test_div_zero_scalar() {
    let tensor1 = Tensor::new(&[1.0, 2.0, 3.0], &[3]);
    let tensor2 = Tensor::new(&[0.0], &[1, 1]);
    let _ = tensor1 / tensor2;
}
...
fn test_div_matrix_and_zero_matrix() {
    ...
    let tensor2 = Tensor::new(&[0.0, 2.0, 3.0, 0.0], &[2, 2]);
    let _ = tensor1 / tensor2;
}
