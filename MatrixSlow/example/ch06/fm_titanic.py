...
# 读取数据，去掉无用列
data = pd.read_csv("../../data/titanic.csv").drop(
...
Embarked = ohe.fit_transform(
    le.fit_transform(data["Embarked"].fillna("")).reshape(-1, 1)
)
...
output = ms.ops.Add(
    ...
    # 二次部分
    ms.ops.MatMul(ms.ops.Reshape(x1, shape=(1, dimension)), ms.ops.MatMul(HTH, x1)),
    b,
)
