...
signal_train = np.array(
    [
        np.array([list(channel) for channel in sample]).T
        for sample in train["relationalAtt"]
    ]
)
signal_test = np.array(
    [
        np.array([list(channel) for channel in sample]).T
        for sample in test["relationalAtt"]
    ]
)
...
label_train = ohe.fit_transform(
    le.fit_transform(train["classAttribute"]).reshape(-1, 1)
)
...
for epoch in range(500):
    ...
    print(
        "epoch: {:d}, accuracy: {:.5f}, train accuracy: {:.5f}".format(
            epoch + 1, accuracy, train_accuracy
        )
    )
