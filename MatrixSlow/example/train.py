...
from matrixslow.ops import Add, Logistic, MatMul, ReLU, Reshape, SoftMax
...
from matrixslow.trainer import (
    DistTrainerParameterServer,
    DistTrainerRingAllReduce,
    Saver,
    SimpleTrainer,
)
...
def plot_data(data_x, data_y, weights=None, bias=None):
    ...
    if weights is not None and bias is not None:
        ...
        bias = np.array(bias)
        if plot_3d:
            y = (-weights[0][0] * x1 - weights[0][1] * x2 - bias[0][0]) / weights[0][2]
...
def build_simple_model(feature_num):
    with ms.name_scope("Input"):
        x = ms.Variable((feature_num, 1), init=False, trainable=False, name="img")

    with ms.name_scope("Hidden"):
        w1 = ms.Variable(
            (HIDDEN1_SIZE, feature_num), init=True, trainable=True, name="weights_w1"
        )
        b1 = ms.Variable((HIDDEN1_SIZE, 1), init=True, trainable=True, name="bias_b1")
        hidden1 = Add(MatMul(w1, x), b1)
        w2 = ms.Variable(
            (HIDDEN2_SIZE, HIDDEN1_SIZE), init=True, trainable=True, name="weights_w2"
        )
        b2 = ms.Variable((HIDDEN2_SIZE, 1), init=True, trainable=True, name="bias_b2")
        hidden2 = Add(MatMul(w2, hidden1), b2)

    with ms.name_scope("Logits"):
        w3 = ms.Variable((CLASSES, HIDDEN2_SIZE), init=True, trainable=True)
...
def build_model(feature_num):
    ...
    with ms.name_scope("Conv"):
        x = Variable(
            (feature_num, 1), init=False, trainable=False, name="img"
        )  # 占位符，28x28 的图像
        img = Reshape(x, (28, 28))
...
def build_metrics(logits, y, metrics_names=None):
    ...
    for m_name in metrics_names:
        metrics_ops.append(
            ClassMining.get_instance_by_subclass_name(Metrics, m_name)(
                logits, y, need_save=True
            )
        )
...
def save():
    ...
    saver.save(
        ...
        service_signature=sig,
    )


def train(
    train_x, train_y, test_x, test_y, epoches, batch_size, mode, worker_index=None
):
    ...
    y = ms.Variable((CLASSES, 1), init=False, trainable=False, name="placeholder_y")
    ...
    if mode == "local":
        ...
        trainer = DistTrainerParameterServer(
            x,
            y,
            logits,
            loss_op,
        ...
        trainer = DistTrainerRingAllReduce(
            x,
            y,
            logits,
            loss_op,
            ...
            metrics_ops=build_metrics(logits, y, ["Accuracy"]),
        )
...
cluster_conf = {
    ...
    # ]
    "ps": ["localhost:5000"],
    "workers": ["localhost:6000", "localhost:6002", "localhost:6004"],
}
...
if __name__ == "__main__":
    ...
    if role == "ps":
        ...
        train_x, train_y, test_x, test_y = util.mnist(
            "D:/develop/project/MatrixSlow/dataset/MNIST"
        )
        mode = args.mode
        phase = args.phase
