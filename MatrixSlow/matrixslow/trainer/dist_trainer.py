...
from ..core import (
    Variable,
    get_trainable_variables_from_graph,
...
class DistTrainerRingAllReduce(Trainer):
    ...
    def __init__(self, *args, **kargs):
        ...
        # 根据集群的环状拓扑结构确定右邻居
        self.target_host = self.workers[(self.worker_index + 1) % self.worker_num]
    ...
    def _optimizer_update(self):
        ...
        for scatter_index in range(self.step):
            ...
            cur_acc_no = (
                self.optimizer.acc_no if scatter_index == 0 else self.recieved_acc_no
            )
    ...
    def _get_gradients_partition(self):
        ...
        self.cur_partion_index = (self.cur_partion_index + self.step) % self.worker_num
        part_gradients = dict()
    ...
    def _wait_for_recieve(self, stage):
        ...
        if self.cond.acquire():
            ...
            # 如果是all-gather阶段则覆盖梯度，样本数保持不变
                ...
                self.optimizer.apply_gradients(
                    self.recieved_gradients,
                    summarize=False,
                    acc_no=self.optimizer.acc_no,
                )
