...
### Phase 5: API 统一（后续，可选）

- [ ] 合并 `forward_batch` 到 `forward`（可选）
- [ ] 更新文档和示例
- [x] **Layer 层设计决策**：Layer 只提供 Batch 版本（详见 `node_vs_layer_design.md`）

> **决策**：Node 层保留两套 API（有意设计），Layer 层只提供 Batch 版本（避免冗余）。
