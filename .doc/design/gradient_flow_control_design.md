...
本文档描述 only_torch 中控制梯度计算和传播的核心机制：`no_grad`、`detach`，以及**可选的** `requires_grad` / 冻结机制。这些机制在高级训练场景（如 GAN、强化学习、多任务学习、迁移学习）中经常组合使用。

> **注意**：动态图架构下，中间结果由 `Rc<NodeInner>` 引用计数管理，天然支持多次 backward，
> 无需 PyTorch 式的 `retain_graph` 参数。详见 [Section 3](#3-多次-backward天然支持)。
...
┌─────────────────────────────────────────────────────────────────┐
│                   多次 backward（天然支持）                       │
│  x → A → B → C → loss1.backward()                               │
│       ↑   ↑   ↑                                                 │
│      Rc 引用计数保持中间结果存活，可再次 backward                   │
│              └───→ loss2.backward()                             │
└─────────────────────────────────────────────────────────────────┘
...
在 PyTorch 中，`backward(retain_graph=True)` 用于在多次反向传播间保留计算图。
only_torch 的动态图架构下，中间结果由 `Rc<NodeInner>` 引用计数管理，
**天然支持多次 backward，无需任何额外参数**。

| 框架 | 多次 backward 的方式 | 内存管理 |
|------|---------------------|---------|
| **PyTorch** | `loss.backward(retain_graph=True)` | 显式控制：不保留则释放 |
| **only_torch** | 直接多次 `loss.backward()` | Rc 引用计数：Var 存在则保留，离开作用域则释放 |
...
#### 场景 1：多 Loss 共享计算路径（多任务学习）

```rust
// 多任务学习：两个 loss 共享 backbone
optimizer.zero_grad()?;
let cls_val = cls_loss.backward()?;  // 第一个 loss
let reg_val = reg_loss.backward()?;  // 梯度自动累积到共享参数
optimizer.step()?;
```
...
optimizer.zero_grad()?;
let actor_val = actor_loss.backward()?;
let critic_val = critic_loss.backward()?;
optimizer.step()?;
```
...
| 机制 | 行为 | 内存 |
|------|------|------|
| Rc 引用计数（默认） | Var 存在时保留中间结果，离开作用域时自动释放 | 自动、高效 |
| 用户的 `zero_grad()` | 清除参数节点的梯度 | 手动控制 |

> **与 PyTorch 的区别**：PyTorch 的 `retain_graph=False` 在 backward 后立即释放中间 buffer，
> 是一种主动内存优化。only_torch 通过 Rust 的 Rc 引用计数实现了等价的效果——
> 当 Var 变量离开作用域时，中间节点自动被释放。两者在正确性上完全等价。
...
**动态图架构下**：中间节点的值由 Rc 引用计数管理，当 Var 离开作用域时自动释放。
每次 backward 开始时会自动清除中间节点的梯度（`reset_intermediate_grad()`），
确保传播信号始终是本次新算的。
...
3. **当前能力已足够**：在 `backward()` 后、下一次 backward 前，中间节点的 grad 是可以访问的，满足大多数调试需求
...
```rust
// backward 后，可以立即访问中间节点的 grad
loss.backward()?;

// 这个时间窗口内，中间节点的 grad 是可访问的
let features_grad = features.grad();
...
|-----------|------------------|
| `test_multi_task_learning` | `tests/multi_task_learning_retain_graph.py` |
