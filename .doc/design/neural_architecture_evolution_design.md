...
**输出语义（当前限制）**：当前 evolution 任务层按单输出模型设计，`BuildResult`、`EvolutionTask::train/evaluate`、loss、metric 和 `EvolutionResult::predict()` 都围绕一个主输出 `Var` 工作。底层 `GraphDescriptor` 支持 `explicit_output_ids: Vec<u64>`，但演化路径目前只写入一个真实输出节点，用于避免 FM 分解/融合产生的无后继中间端点被误判为输出。多输出 / 多头任务需要单独设计 output head 标记、target/loss/metric 映射和结果返回结构，见 11.7 阶段 D roadmap。
...
| **多输出 / 多头任务演化** | 中高 | 任务层扩展，底层 IR 已部分具备 | 当前 evolution 只支持单主输出；未来可扩展为 `outputs: Vec<Var>`，并在 genome 中显式标记多个 output head。需要定义每个输出对应的 target、loss、metric、权重聚合策略，以及 `EvolutionResult::predict()` 的多输出返回结构。适用：多任务学习、辅助头监督、actor-critic 联合搜索、检测/分割多头输出 |
