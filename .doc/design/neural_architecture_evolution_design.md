...
Evolution::supervised(train, test, TaskMetric::Accuracy)
    ...
    .with_initial_portfolio(InitialPortfolioConfig::vision_classification()) // 覆盖初始候选族（空间分类默认启用）
    .with_candidate_scoring(CandidateScoringConfig::p5_lite()) // 覆盖 P5-lite 预筛（空间分类默认启用）
...
默认演化会从任务类型推导一个 `minimal_genome`，再通过随机爆发变异生成初始种群。空间分类任务会默认混入多个合理结构族，让它们进入同一套训练、评估、Pareto 选择流程；高级用户仍可通过 `with_initial_portfolio(...)` 覆盖或关闭。
...
`CandidateScoringConfig::p5_lite()` 是完整 surrogate 之前的低耦合预筛层。空间分类任务默认启用；其他任务域暂时保持关闭，直到有稳定候选族和验证数据。它不会预测最终 fitness，也不会替代训练评估，只基于结构特征和 FLOPs 对候选排序，并将完整训练预算集中到 top-k。
...
P5-lite 预筛使用 family-diverse top-k：先按启发式 score 排序，再确保 `FlatMLP`、`TinyCNN`、`LenetLike`、`Hybrid`、`Other` 等结构族尽量各保留至少一个候选，避免低 FLOPs 的 FlatMLP 在早期把 CNN 家族全部挤出完整训练评估。
...
| Spatial 域演化运行偏慢 | 部分改善 | 已完成多项优化：(1) FM 掩码融合；(2) `SizeConstraints::auto()` 空间域调优；(3) `ComplexityMetric` 默认 FLOPs；(4) BLAS 线程守卫；(5) backward 路径减少无用梯度、SoftmaxCrossEntropy 融合、Conv2d im2col 复用；(6) 初始候选族、family-diverse P5-lite 与 ASHA 多样性保护。MNIST search 已达标，segmentation evolution 仍需继续纳入同一专项 |
