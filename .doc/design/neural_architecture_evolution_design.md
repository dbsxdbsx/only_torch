...
固定数量多头任务仍属于 supervised，只是从快捷入口升级到显式 `SupervisedSpec`：
...
完整示例见 `examples/evolution/multi_head_quadrant_radius`。P3 第一阶段限定为共享输入、每个 head target 与 inputs 一一对齐；不支持不同 head 使用不同 dataset、缺标注 masking 或 detection/instance matching。
...
Evolution::supervised(train, test, TaskMetric::Accuracy)
    ...
    .with_batch_size(64)              // 显式 batch size（默认自动策略；空间分类为 128）
    .with_initial_burst(2)            // 初始种群随机爆发变异次数（默认按约束自动推导）
    .with_initial_portfolio(InitialPortfolioConfig::vision_classification()) // 覆盖初始候选族（空间分类 / segmentation 默认启用）
    .with_candidate_scoring(CandidateScoringConfig::p5_lite()) // 覆盖 P5-lite 预筛（空间分类 / segmentation 默认启用）
    ...
    .with_asha(AshaConfig::default()) // ASHA 多保真评估（非序列 Phase 1 默认启用）
...
初始结构：默认从 minimal genome 出发；空间分类默认混入多个任务合理初始候选族（FlatMLP / TinyCNN / LeNetTiny）
随机爆发初始化：对非候选族种子个体施加 K 次随机变异（默认 K = max(2, min(8, max_layers/2))，可通过 with_initial_burst 覆盖）生成初始种群
初始候选达标直返：初始候选族评估后若已有 primary ≥ target_metric，直接返回 TargetReached，避免进入随机变异长跑
...
默认演化会从任务类型推导一个 `minimal_genome`，再通过随机爆发变异生成初始种群。空间分类任务会默认混入多个合理结构族；dense segmentation 任务默认混入最小分割头与稍深 dense conv head，让它们进入同一套训练、评估、Pareto 选择流程。高级用户仍可通过 `with_initial_portfolio(...)` 覆盖或关闭。

这里的 Initial Portfolio 只是**结构候选族初始化**，不是预训练权重 warm start。权重层面的复用仍由 Lamarckian 权重继承负责。
...
| `spatial_segmentation_tiny` | `Conv → ReLU → Conv → ReLU → 1×1 Conv head` | 稍深 dense 分割候选族，保持 H/W 输出协议 |

Portfolio 不是写死答案：候选仍然必须经过训练和评估，只有 primary / cost 表现足够好的分支会进入 parents 或 archive。若初始候选族已经存在达标候选，`run()` 会直接返回 `TargetReached`，避免把用户默认路径拖入随机长搜索；若未达标，则继续进入常规 mutation loop。

### 3.4 P5-lite Candidate Scoring

`CandidateScoringConfig::p5_lite()` 是完整 surrogate 之前的低耦合预筛层。空间分类任务与 segmentation 审计路径默认启用；其他任务域暂时保持关闭，直到有稳定候选族和验证数据。它不会预测最终 fitness，也不会替代训练评估，只基于结构特征和 FLOPs 对候选排序，并将完整训练预算集中到 top-k。
...
- P5-lite 生成池与保留池的结构族分布；
- 全量候选 score 分布与 kept score 分布；
- 真实评估候选的结构族分布；
- 本批最佳候选来自哪个结构族。

P5-lite 预筛使用 family-diverse top-k：先按启发式 score 排序，再确保 `FlatMLP`、`TinyCNN`、`LenetLike`、`Hybrid`、`DenseSegHead`、`DenseSegDeep`、`EncoderDecoderSeg`、`Other` 等结构族尽量各保留至少一个候选，避免低 FLOPs 的 FlatMLP 或浅层 dense head 在早期把其他结构族全部挤出完整训练评估。

结构族统计通过通用计数容器输出与查询，不在 public `CandidateFamilyCounts` 上为每类任务追加专用字段。`DenseSegHead` / `DenseSegDeep` / `EncoderDecoderSeg` 只是内部候选族标签，用于 P5-lite 与 ASHA 多样性保护。

这为后续完整 P5 surrogate 积累 `(genome_features → fitness / cost / timing)` 数据，但当前不引入学习型回归器，避免在样本不足时把主循环复杂化。完整 learned surrogate 暂停作为主线；只有当真实 benchmark 证明评估成本成为瓶颈，并且结构特征与真实 fitness 呈稳定正相关时，才应把 P5-lite 升级为任务本地的学习型 surrogate。

ASHA 同样做轻量多样性保护：中间 rung 先保留全局 top elite，再尽量保留不同结构族代表，并通过 `min_survivors` 防止小候选池在末轮只剩单一结构族。这解决了 MNIST search 中“CNN 候选进入 P5-lite 保留池，但被 1 epoch 低保真 ASHA 过早淘汰”的问题。
...
**输出语义**：`NetworkGenome` 可记录命名 `OutputHead`，`BuildResult` 同时保留旧的 `output: Var` 默认输出和新的 `outputs: Vec<Var>` 多输出列表。旧单输出 genome 仍回退到最后一个非参数节点；多头 supervised genome 则显式写入多个 output head，避免 FM 分解/融合产生的无后继中间端点被误判为输出。P3 第一阶段限定为平坦共享输入的固定多头 supervised evolution，空间 / 序列多头、检测变长实例、matching / NMS / mAP 等协议仍留到后续任务。

**多头 supervised 协议**：旧入口 `Evolution::supervised(train, test, metric)` 完全兼容，内部包装成单 head `SupervisedSpec`。多头任务使用 `Evolution::supervised_task(SupervisedSpec::new(train_inputs, test_inputs).head_targets(...).primary_head(...))`；每个 head 拥有独立 targets、`TaskMetric`、loss/metric 权重和 inference 标记。训练时逐 head 创建 target/loss 并按 `loss_weight` 聚合；评估时逐 head 生成 `HeadMetricReport`，`FitnessScore.primary` 默认取 primary head，未指定时回退为 weighted metric 平均。`EvolutionResult::predict()` 继续返回默认 head，`predict_head()` / `predict_heads()` 支持只推理所需 head。
...
NodeLevel 中不再维护独立的跳跃边概念。任何跨层连接（残差、绕连等）都是 DAG 中的普通前向父边，由 `AddConnection`/`RemoveConnection` 变异直接操作。
...
| `InsertEncoderDecoderSkip` | 0.04 | ✅ | Segmentation 专用，同 Phase 1，但精炼阶段降低权重 |
...
| `FixedEpochs(n)` | 快速筛选候选架构（序列任务 Phase 1 默认路径，或关闭 ASHA 后使用；`n = max(3, min(10, n_train/(batch_size*5)))`） |

演化主循环自动分配两阶段训练预算：
- **Phase 1（前 40% 代数）**：非序列任务默认使用 ASHA 多保真评估（`rung_epochs=[1,2,4], eta=3`），序列任务默认回退到 `FixedEpochs(fast_epochs)` 以避免极短 early rung 在悬崖型 loss 上误淘汰候选；用户可通过 `.with_asha(None)` 关闭 ASHA
- **Phase 2（后 60% 代数）**：`UntilConverged`（用户 `ConvergenceConfig`）— 对有潜力拓扑做充分训练

**设计注：为什么序列 / 记忆单元默认跳过 ASHA？** ASHA 的前提是“低预算排序能预测高预算表现”。Flat / Spatial 任务通常较满足这一点；但 RNN/LSTM/GRU 等记忆任务在早期 epoch 往往所有候选都接近随机表现，尤其 parity / 变长序列这类悬崖型任务，1 epoch rung 很容易把慢热但正确的结构提前淘汰。当前默认跳过是基于既有实验问题的保守策略，不代表 ASHA 机制不能扩展到序列域；后续可通过 sequence-aware rung（更长 warmup、延迟淘汰、结合 loss slope proxy）重新启用。
...
5. （可选）在内部 `LayerSpec` 枚举中添加层规格入口，并在层展开函数中处理展开
...
初始使用最简单的 Rnn，`MutateCellType` 可升级为 LSTM/GRU，`InsertLayer` 可在序列域再插入更多循环层块。
...
当前系统在存储层统一使用节点粒度表示（`Vec<NodeGene>`），但所有变异操作的实际粒度仍为 **层块（Block）级别**——`InsertLayer` 调用 `expand_linear()` / `expand_conv2d()` / `expand_rnn()` 等函数，一次性插入整个多节点层块（如 Linear = MatMul + Add + 2×Parameter）。这相当于在"器官"级别做移植，而非在"细胞"级别做增减。

**唯一内核表示**：系统只有 NodeLevel 一种运行时表示。所谓层块级变异和节点级变异并非两套独立系统，而是同一个 NodeLevel DAG 上不同粒度的操作——层块变异（InsertLayer 等）一次操作多个节点的层块，原子节点变异（InsertAtomicNode）一次操作单个节点，循环边变异（AddRecurrentEdge / RemoveRecurrentEdge）操作节点间的时延连接。三者在同一个 `Vec<NodeGene>` 上共存，通过 `MutationRegistry` 的权重配比控制使用比例。无需维护"层级模式"与"节点模式"的切换——粒度是变异操作的属性，不是基因组的属性。
...
| 方向 | 状态 | 实现说明 |
|---|---|---|
| Conv-BN-ReLU 复合层块 | ❌ 已移除 | 实测表明演化更倾向于独立发现最优激活组合（如 LeakyReLU + Swish），硬编码 Conv+BN+ReLU 限制了搜索多样性。改为依赖现有的独立插入机制：30% 概率插入任意激活函数 + 100% Conv2d 作为 Spatial 域默认插入。BatchNorm 的独立插入可在后续按需加入 |
...
| Conv 超参变异扩展（stride） | ✅ | `MutateStrideMutation`：在 stride (1,1) 和 (2,2) 之间切换，允许卷积层自身进行空间降维。已在 Phase 1/2 注册表中注册（权重 0.06）。Dilation 已在阶段 C 作为 Conv2d 空洞卷积能力补齐 |
...
覆盖非空间世界（Flat + Sequence），目标是达到与 NEAT（2002）/ EXAMM（2019）同等的变异粒度。层块变异（InsertLayer 等）继续保留，作为"大步跳跃"的探索手段；原子节点变异作为"精细雕刻"的新增能力，两者通过注册表权重共存。
...
| 归一化层插入（B 补强） | ✅ | InsertLayer 新增 10% 概率插入归一化层块：**BatchNorm**（Flat/Spatial 域，5 节点：gamma + BN_op + Mul + beta + Add）、**LayerNorm**（Flat/Sequence 域，5 节点：gamma + LN_op + Mul + beta + Add）、**RMSNorm**（Flat/Sequence 域，3 节点：gamma + RMSNorm_op + Mul）。归一化块为 shape passthrough（不改变维度），不可 resize（Grow/Shrink 自动跳过），参数形状随上游维度自动级联修复。避免连续归一化块 |
...
|------|------|----------------|------|
| **Deformable Conv2d** | 中 | 正交，新 NodeTypeDescriptor 变体 | 采样位置可学习的卷积。当前已实现 offset-only RawNode / Layer / NodeLevel block，并通过 `evolution_deformable_conv2d_segmentation` 验证 DeformableConv2d dense segmentation 初始族能进入 evolution 主流程。FM 边类型扩展（`ChangeFMEdgeType`）保留到确认命中率不足后再做 |
| **Dynamic Conv2d** | 中 | 正交，新节点类型 | kernel 权重由输入动态生成（通过一个小型 attention 或线性层），而非静态参数。实现为新 RawNode + 专属层规格扩展。应用：轻量级模型中用少量参数表达更丰富的卷积行为 |
...
| **多输出 / 多头任务演化** | 中高 | P3 第一阶段已完成 | 已支持固定数量命名 head 的 supervised evolution：`SupervisedSpec` / `HeadSpec`、genome output head 元数据、`BuildResult.outputs`、逐 head loss/metric/report 与 `predict_head(s)`。当前限制为平坦共享输入、所有 head target 与 inputs 对齐；空间 / 序列多头、检测/实例 matching、NMS 与 mAP 仍是后续扩展 |
...
// 3. 当前先通过 NodeLevel block / 初始候选族接入 segmentation evolution
//    若后续确认命中率不足，再让 ChangeFMEdgeType 覆盖 conv ↔ deformable_conv
...
| **P5-lite / Surrogate 模型** | 中 | 解耦，效率优化 | 当前已落地 P5-lite：记录结构特征并用启发式 scorer 预筛 top-k，完整 fitness 仍由真实训练评估产生。完整 learned surrogate 暂停为研究项：只有在真实 benchmark 证明评估成本足够高、且 audit 相关性可靠时，才训练任务本地轻量 predictor |
...
├─ 10 种变异           └─ ASHA 多保真 ✅       ├─ 多输出/多头任务     └─ DARTS 混合搜索
└─ Builder 优化                                └─ Dynamic Conv

已完成                已完成                   按需添加                长期方向
（空间域基础设施）       （已修复流程内在缺陷）     （扩展算子多样性）        （扩展搜索效率）
```

> **说明**：阶段 A–C 建立各域变异基础设施；阶段 F 修复了评估与继承两条链路的内在缺陷（破坏性变异 + 弱选择信号 + 均匀低预算）。A–C、F 全部落地后，当前演化流程已自洽；其中 ASHA 在非序列任务默认启用，序列任务默认保留 FixedEpochs 路径。阶段 D 的所有新算子都是"插件"，通过 `MutationRegistry` 和 `NodeTypeDescriptor` 自然接入；阶段 E 的效率优化不影响搜索能力，可在任何时间点独立引入。
...
| **ASHA 多保真评估** | ✅ | Flat / Spatial 默认启用；Sequential 默认跳过 | `AshaConfig` 默认 `rung_epochs=[1,2,4], eta=3`：rung 0 所有 offspring 训练 1 epoch，保留 top 1/3；rung 1 幸存者再训练 2 epoch，保留 top 1/3；rung 2 最后训练 4 epoch。Lamarckian 权重快照天然支持跨 rung 继续训练。非序列任务头部候选可获得更高训练预算，总预算基本持平。序列任务默认跳过 ASHA，避免 parity/RNN 等悬崖型 loss 在 1 epoch early rung 中缺乏区分度而误淘汰结构候选；可通过自定义 `AshaConfig` 手动启用或 `.with_asha(None)` 关闭 |
| **Task-aware minimal genome** | ✅ | 序列域优先 | `minimal_sequential` 从 hidden = output_dim 提升为 `max(4, output_dim)`，解决 parity 类问题状态容量不足 |
| **MetricReport 评估报告** | ✅ | 三域通用 | `FitnessScore::report` 保存 primary head 的附加指标；多头任务额外填充 `head_reports` 保存逐 head primary / tiebreak / report。报告指标只用于观测，不进入 `objective_point()`、target 或 archive 收敛判断 |

ASHA 的实现路径仍然是通用评估调度器；当前差异只在默认策略。序列域默认跳过是为了避免重复早期 parity_seq 实验中“低预算信号不足导致结构候选被过早淘汰”的问题。若未来加入 sequence-aware ASHA 配置并通过固定长度 / 变长序列回归测试，可以再把默认策略推进到真正的三域启用。
...
4. ✅ ASHA 多保真评估（F4）——系统性提升非序列头部候选的训练预算

阶段 F 完成后，fitness 信号已足够可靠，后续优先进入阶段 D（新算子扩展）和真实 benchmark 闭环；阶段 E 的完整 learned surrogate 暂停为可选研究项，避免在小型 CPU benchmark 上过度工程。
...
5000 train / 1000 test
  ...
  P5-lite：生成候选池后按结构特征 + FLOPs 预筛 family-diverse top-k
  若初始候选族未达标，再进入：
    Phase 1：Pareto 种群搜索 + ASHA 多保真评估 + 结构探索变异权重
    Phase 2：Pareto 种群搜索 + UntilConverged 充分训练 + 超参调优变异权重
```

MNIST 示例不再暴露 `smoke / quality / audit / search` profile。最终用户路径就是默认搜索路径，框架内部自动选择已验证的空间分类策略。

2026-04-28 当前验证：`evolution_mnist` 在 5000 train / 1000 test 下，默认路径 5 个 seed 全部达标：seed 1..5 的 Accuracy 为 95.8% / 95.0% / 95.0% / 95.0% / 95.4%，总耗时约 30.2s / 19.7s / 33.2s / 41.1s / 11.5s。这说明 MNIST search 的主要卡点已从“找不到 95% 候选”收敛为“如何把同样的结构族与多保真策略迁移到 segmentation evolution 慢路径”。
...
### Segmentation Evolution (input=1@32×32, output=classes@32×32)
...
24 train / 8 test
  ...
  P5-lite：按 dense / encoder-decoder segmentation 结构族 + FLOPs 预筛 family-diverse top-k
  Phase 1：ASHA 多保真评估 + spatial-to-spatial / encoder-decoder skip 变异（不迁移到 FM level）
```

2026-04-28 当前验证：`evolution_overlapping_shapes_semantic_segmentation` 在 24 train / 8 test 下，默认 seed=42 达到 Mean IoU 63.0%，总耗时为秒级。审计日志显示初始评估包含 `seg_head=1 / seg_deep=2`，P5-lite 在第 0 代生成 `seg_head=3 / seg_deep=9` 并保留 `seg_head=2 / seg_deep=2`，ASHA 后最佳来自 `dense_seg_deep@0.630`。当前最小 segmentation evolution 慢路径已闭环，剩余成本不是 scorer 或 ASHA；`loss_var.backward()` 计时已拆成 `backward_total`、`backward_forward`、`backward_propagate`，本轮证据显示主要剩余成本在 ensure-forward。BCEWithLogits 前向已改为单次扫描生成 sigmoid 缓存与稳定 loss，并去掉 target 缓存 clone；`Conv2d` forward 对 padding 为 0 的 1x1 / valid conv 不再深拷贝输入作为 `padded_input`。最新单次 debug + BLAS 复测约 2.9s。传统对照已新增 `overlapping_shapes_unet_lite_segmentation`，在 64x64 overlapping shapes benchmark 上约 27.4s 达到 Mean IoU 75.6%；下一步若继续性能专项或扩大 evolution benchmark，应优先对 dense Conv2d / ConvTranspose2d 的 im2col / GEMM / padding 分支做更细粒度 profiling。

同日补充：`evolution_overlapping_shapes_unet_lite_segmentation` 已把 evolution benchmark 对齐到 64x64 / 4 类 / 0..3 个可重叠形状数据，并将 `spatial_segmentation_unet_lite` encoder-decoder + skip concat 种子纳入默认 segmentation portfolio 与 `encoder_decoder_seg` P5-lite 家族。随后补入 `InsertEncoderDecoderSkipMutation`，可在 segmentation Phase 1/2 中插入保持分辨率的 Pool / Deconv / channel concat skip / fuse conv 局部结构；该变异已通过 GraphDescriptor 输出形状与 buildability 测试。target Mean IoU 0.60 下的 5-seed 稳定性验证已闭环：seed 1..5 全部 `TargetReached`，Mean IoU 为 93.3% / 98.4% / 90.3% / 77.1% / 63.0%，耗时约 159.8s / 169.2s / 131.5s / 15.0s / 28.6s。该结果说明同类数据规模下 evolution 成本仍可接受，且搜索空间已能表达并变异 U-Net-lite 局部结构；后续如果继续提升，应优先看训练预算、mutation 命中率和 Conv2d / ConvTranspose2d profiler，而不是立即升级完整 P5 surrogate。

同日补充：P5 learned surrogate 暂停作为主线，P4+ 继续补齐 DeformableConv2d evolution 闭环。新增 `spatial_segmentation_deformable_tiny` 初始基因组与 `InitialPortfolioConfig::vision_segmentation_with_deformable()`；`evolution_deformable_conv2d_segmentation` 使用 16x16 二值前景分割小 benchmark，显式启用 DeformableConv2d 初始族并关闭 P5-lite。随后修复 DeformableConv2d 运行时动态 batch 与 BCE logits 下 BinaryIoU 标签阈值问题；默认 seed=42 在 4 个测试样本上 `TargetReached`，Binary IoU 40.3%，最终基因组包含 DeformableConv2d，总耗时约 1.0s。当前暂不扩展 `ChangeFMEdgeType` 到 deformable FM edge；若后续更大 benchmark 显示 Deformable 候选命中率不足，再把它作为搜索空间增强推进。
...
| 问题 | 状态 | 说明 |
|---|---|---|
| MNIST 准确率上限 ~91.5% | ✅ 已修复 | 空间分类默认启用初始候选族、family-diverse P5-lite、ASHA 多样性保护与 final_refit；`evolution_mnist` 默认路径在 5000 train / 1000 test 下 5 个 seed 全部达到 95% |
| Spatial 域演化运行偏慢 | ✅ 当前最小闭环已修复 | 已完成多项优化：(1) FM 掩码融合；(2) `SizeConstraints::auto()` 空间域调优；(3) `ComplexityMetric` 默认 FLOPs；(4) BLAS 线程守卫；(5) backward 路径减少无用梯度、SoftmaxCrossEntropy 融合、Conv2d im2col 复用；(6) 初始候选族、family-diverse P5-lite 与 ASHA 多样性保护；(7) dense segmentation portfolio 与内部 `DenseSegHead` / `DenseSegDeep` 候选族观测；(8) `backward_total` 拆分 ensure-forward 与 backward-propagate，BCEWithLogits 前向融合 loss / sigmoid 缓存；(9) Conv2d 无 padding forward 避免深拷贝输入；(10) encoder-decoder segmentation seed、`InsertEncoderDecoderSkipMutation` 与内部 `EncoderDecoderSeg` 候选族观测。MNIST search 已达标，当前最小 segmentation evolution 秒级达标；更大分割 benchmark 若变慢，应继续沿同一 timing 矩阵排查 dense Conv2d / ConvTranspose2d im2col / GEMM / padding / backward |
| Evolution 多输出模型 | ✅ 第一阶段已完成 | 已支持固定数量命名多头 supervised evolution：`SupervisedSpec` / `HeadSpec`、`OutputHead` 元数据、`BuildResult.outputs`、逐 head loss/metric/report 与选择性推理。当前限制为平坦共享输入；空间/序列多头与 detection/instance matching 后续推进 |

---

*本文档描述 only_torch 的神经架构演化模块实际实现。最后更新：2026-04-28（v23: 补充 segmentation encoder-decoder skip 结构变异）*
