...
同日补充：P5 learned surrogate 暂停作为主线，P4+ 继续补齐 DeformableConv2d evolution 闭环。新增 `spatial_segmentation_deformable_tiny` 初始基因组与 `InitialPortfolioConfig::vision_segmentation_with_deformable()`；`evolution_deformable_conv2d_segmentation` 使用 16x16 background / foreground 二类语义分割小 benchmark，显式启用 DeformableConv2d 初始族并关闭 P5-lite。默认 seed=42 在第 2 代 `TargetReached`，Mean IoU 58.8%，最终基因组包含 DeformableConv2d，总耗时约 34.4s。当前暂不扩展 `ChangeFMEdgeType` 到 deformable FM edge；若后续更大 benchmark 显示 Deformable 候选命中率不足，再把它作为搜索空间增强推进。
