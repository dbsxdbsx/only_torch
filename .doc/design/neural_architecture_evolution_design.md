...
24 train / 8 test
  ...
  Phase 1：ASHA 多保真评估 + spatial-to-spatial 变异（不迁移到 FM level）
```
...
同日补充：`evolution_overlapping_shapes_unet_lite_segmentation` 已把 evolution benchmark 对齐到 64x64 / 4 类 / 0..3 个可重叠形状数据，并将 `spatial_segmentation_unet_lite` encoder-decoder + skip concat 种子纳入默认 segmentation portfolio 与 `encoder_decoder_seg` P5-lite 家族。当前 debug + BLAS 单次复测约 20.0s，达到 Mean IoU 53.3%，并输出 input / target / prediction 三张代表样本图。该结果说明同类数据规模下 evolution 成本仍可接受，且搜索空间已能表达 U-Net-lite 起点；但与 U-Net-lite 手写强基线仍有约 22 个百分点的 Mean IoU 差距，后续更值得验证的是 Pool / Deconv / skip concat 的结构变异与训练预算，而不是立即升级完整 P5 surrogate。
...
*本文档描述 only_torch 的神经架构演化模块实际实现。最后更新：2026-04-28（v22: 补充 MNIST 默认路径 5-seed 稳定性验证）*
