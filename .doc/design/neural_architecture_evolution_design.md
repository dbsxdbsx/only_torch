...
Evolution::supervised(train, test, TaskMetric::Accuracy)
    ...
    .with_initial_portfolio(InitialPortfolioConfig::vision_classification()) // 覆盖初始候选族（空间分类 / dense segmentation 默认启用）
    .with_candidate_scoring(CandidateScoringConfig::p5_lite()) // 覆盖 P5-lite 预筛（空间分类 / dense segmentation 默认启用）
...
`CandidateScoringConfig::p5_lite()` 是完整 surrogate 之前的低耦合预筛层。空间分类任务与 dense segmentation 审计路径默认启用；其他任务域暂时保持关闭，直到有稳定候选族和验证数据。它不会预测最终 fitness，也不会替代训练评估，只基于结构特征和 FLOPs 对候选排序，并将完整训练预算集中到 top-k。
...
P5-lite 预筛使用 family-diverse top-k：先按启发式 score 排序，再确保 `FlatMLP`、`TinyCNN`、`LenetLike`、`Hybrid`、`DenseSegHead`、`DenseSegDeep`、`Other` 等结构族尽量各保留至少一个候选，避免低 FLOPs 的 FlatMLP 或浅层 dense head 在早期把其他结构族全部挤出完整训练评估。

结构族统计通过通用计数容器输出与查询，不在 public `CandidateFamilyCounts` 上为每类任务追加专用字段。`DenseSegHead` / `DenseSegDeep` 只是内部候选族标签，用于 P5-lite 与 ASHA 多样性保护。
...
24 train / 8 test
  ...
  框架默认：Initial portfolio = minimal dense head + dense segmentation tiny
  P5-lite：按 dense segmentation 结构族 + FLOPs 预筛 family-diverse top-k
  Phase 1：ASHA 多保真评估 + dense spatial-to-spatial 变异（不迁移到 FM level）
```
...
同日补充：`evolution_overlapping_shapes_unet_lite_segmentation` 已把 evolution benchmark 对齐到 64x64 / 4 类 / 0..3 个可重叠形状数据，仍使用现有 dense spatial-to-spatial segmentation evolution，不要求直接搜索 U-Net encoder-decoder。当前 debug + BLAS 单次复测约 24.5s，达到 Mean IoU 54.3%，并输出 input / target / prediction 三张代表样本图。该结果说明同类数据规模下 evolution 成本已可接受，但与 U-Net-lite 手写强基线仍有约 21 个百分点的 Mean IoU 差距；后续更值得验证的是 Pool / Deconv / skip concat 搜索空间，而不是继续只调当前 dense head 的候选预算。
