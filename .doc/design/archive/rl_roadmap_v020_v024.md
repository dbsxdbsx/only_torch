# 强化学习路线图（v0.20–v0.24 历史归档）

> **📦 本文档为历史归档（2026-07-02 起冻结）**：记录 v0.20–v0.24「每版一个算法」时代（SAC → AlphaZero → MuZero/PPO → EfficientZero V2）的设计决策、实施计划与技术债清单。
> 其中的目录树、组件状态、`Step` 待删 / legacy gym 清理等描述**均为当时快照**，与当前代码可能不符；文中 `examples/muzero/`、`examples/efficientzero/`、`examples/alphazero/` 等路径已不存在。
> **当前 RL 状态与路线**请读 [rl_roadmap.md](../rl_roadmap.md)（薄版当前态）。仍然有效的长期内容（MCTS 接缝设计 §2.5、能力边界 §2.2.1b、SAC 技术笔记 §4）由新版按需链回本文。
>
> **创建日期**：2026-02-14 · **归档日期**：2026-07-02
> **归档时状态**：v0.24.0 已发版；v0.25 MyZero 统一算法开发中（见新版 roadmap）
...
| **SAC-Discrete** | `examples/sac/cartpole/` | **CartPole-v1**（架构跑通；发版验收 **greedy eval ≥ 475**；2026-06-16 统一 v1、废弃 v0） |
...
|-------------|------|------|-------------------------|
| **向量** | **CartPole-v1**（SAC/MuZero/PPO 架构验收 greedy≥475；2026-06-16 统一 v1）、LunarLander-v3（离散） | Pendulum-v1、MountainCarContinuous-v0 | **`Platform-v0`**（`hybrid-platform`，见 §2.2.2） |
...
参照：mctx（泛型 `search` + 可插拔 `muzero_policy`/`gumbel_muzero_policy`）、[LightZero](https://github.com/opendilab/LightZero)（tree / policy / buffer 分层，按变体层叠）。**Rust 单份树**即 LightZero 的 ctree，省掉其 Python/C++ 双份维护。签名细节见 RL 主线实施计划 v0.22 §C（本机 plan，未随仓库分发）。
...
- **新增** `pub struct Transition`（字段：`obs, action: Vec<f32>, reward, next_obs, terminated, truncated`——**镜像 Gymnasium，勿合并成单一 `done`**；TD target 用 `1 - terminated` bootstrap，CartPole-v1 撞 500 步是 `truncated` 仍需 bootstrap）
...
|------|------|---------------|------|------|
| **架构跑通** | SAC、MyZero、PPO | **`CartPole-v1`**（满分 500，solved = **475**） | 发版 / 示例：**greedy(temp=0) eval 均值 ≥ 475** | v0.20 **smoke** 不断言 reward，只验管线。**2026-06-16 全项目统一 v1、废弃 v0**（v0 仅 DeprecationWarning + v1 是 MyZero 对标基准）；MyZero greedy 噪声大、首达可能 spike |
| **终极调优** | **MyZero**（项目**唯一**的 `*Zero` 实现，学术上承接 [EfficientZero V2](https://arxiv.org/abs/2403.00564) 谱系） | **`-v1` / 新版 ID** | 各任务专属指标（侧重样本效率 env-steps） | 离散+连续+视觉+低维；混合 Tuple 为工程扩展 |

端到端表见 RL 主线实施计划（本机 plan，未随仓库分发）。

### 5.7 MuZero + PPO（**v0.23 主线**）

> ⚠️ **历史记录（已被 §8 MyZero 取代）**：v0.23 的独立 `muzero/` 模块与示例**已删除**，组件全部吸收进 `src/rl/algo/my_zero/`（项目唯一的 `*Zero` 实现）。本节仅留作演进溯源，文中 `examples/muzero/` 等路径已不存在；PPO（model-free 基线）不受影响，仍在 `examples/ppo/`。
>
...
引入 **learned dynamics** 与 **on-policy buffer**（与 SAC 共用 **CartPole-v1 greedy eval ≥ 475** 标准，见 §5.6.1；2026-06-16 统一 v1）：
...
| MuZero 示例 | `examples/muzero/cartpole/` | **`CartPole-v1`**（2026-06-16 统一 v1；v0.23.1 时代为 v0 greedy 199.5）；**canonical 完全体**：categorical value/reward + latent min-max 归一化 + absorbing state + canonical 梯度缩放（`Var::scale_gradient`）+ `MuZeroConfig`（按环境配 sim）+ `reanalyze_game`（默认关）|
| PPO 示例 | `examples/ppo/cartpole/` | **`CartPole-v1`**（2026-06-16 统一 v1）；`Rollout`/`RolloutBuffer` + `src/rl/algo/ppo/`；**greedy eval ≥ 475** |

> **v0.23.1 收口（2026-06-15）**：MuZero 平台期真因是搜索的 **no-terminal 价值膨胀**（非 categorical 缺失），由 canonical absorbing state 修复后达标。`scale_gradient` / `MuZeroConfig`(按环境 sim) / `reanalyze` 机制均已入库（`*Zero` 家族共享地基）；独立 target network 属 EZ 增强、不在 base MuZero。详见 [`.issue/_archive/muzero_cartpole_scalar_value_plateau.md`](../../.issue/_archive/muzero_cartpole_scalar_value_plateau.md)。

### 5.8 EfficientZero V2 终极调优 + 多模式矩阵（**v0.24 主线**）

> ⚠️ **历史记录（已被 §8 MyZero 取代）**：v0.24 的独立 `efficientzero/` 模块与六格示例矩阵**已删除**，组件（consistency / value_prefix / target_net / SVE 等）全部吸收进 `src/rl/algo/my_zero/`。本节仅留作演进溯源，文中 `examples/efficientzero/` 等路径已不存在；MyZero 在学术上承接 EZ-V2 谱系（论文出处见 [MyZero 总览](../../examples/my_zero/README.md) 脚注）。
>
...
>
> v0.24 起性能调优只落在 EZ-V2；环境 ID **一律 `-v1` / 新版**。（**2026-06-16**：架构层 CartPole 也已统一 v1，与 EZ-V2 共用 v1 环境，差异仅在验收指标——架构层看 greedy≥475、EZ-V2 看样本效率。）EZ-V2 在 V1 的 value prefix / reanalyze 之上增加 **Gumbel 连续动作搜索**、**SVE** 等。

| 模式 | 示例 | 环境 ID | 验收 |
|------|------|---------|------|
| 完美信息博弈 | `examples/efficientzero/gomoku/` | `Gomoku-*-v0` | 同训练量胜率 ≥ AlphaZero |
| 向量离散 | `examples/efficientzero/cartpole/` | **`CartPole-v1`** | EZ 任务指标（样本效率 env-steps；分数天花板 500 已达） |
...
>
> **v0.20 之后的版本路线**：§5.1 v0.21 示例瘦身 / §5.5 v0.22 双 trait / §5.6 v0.22 AlphaZero / **§5.7 v0.23 MuZero + PPO** / **§5.8 v0.24 EfficientZero V2 + 多模式矩阵**；端到端表参见 RL 主线实施计划（本机 plan，未随仓库分发）。
>
...
>
> **Creator 环境宪法（2026-05-27）**：`python/gym_env/<游戏>/`；MCTS 在库、Board 在 Python；详见 RL 主线实施计划 v0.22（本机 plan，未随仓库分发）。
...
## 8. v0.25 MyZero 统一算法（2026-06-16 方向定稿）

> **战略转向**：v0.20–v0.24 采用「每版一个算法」分治路线（SAC → AlphaZero → MuZero/PPO → EfficientZero V2），各算法独立实现、独立达标。v0.25 起转为 **MyZero 统一算法**——在 `src/rl/algo/my_zero/` 内，从最简 base 开始，以**消融实验**方式逐增量叠加组件，最终覆盖全动作空间（离散/连续/混合）与全状态类型（确定/随机），成为唯一的 `*Zero` 族实现。  
> **战略层文档**：[MyZero 算法纲领](./my_zero_algorithm_vision.md)（做/不做、文献谱系）· **实测矩阵**：[MyZero 示例总览](../../examples/my_zero/README.md)

### 8.1 设计理念

- **一个算法，持续迭代**：不再为每篇论文建独立目录，而是维护一个不断进化的 MyZero。
- **消融驱动**：每叠一个增量（consistency / value prefix / target net / SVE / Gumbel），必须在 CartPole-v1 上做 A/B 消融，记录 benchmark 基线与增量贡献。
- **从简到繁**：CartPole（离散向量）→ Pendulum（纯连续）→ Platform（混合）→ Gomoku（博弈）→ Atari（图像）→ Ant（高维连续）。
- **组件公共化**：可复用组件（categorical value encoding、value transform、n-step target 等）现自洽于 `my_zero` 内；若未来需与其他算法共享，再上提到 `mcts/` 或 `algo/common/` 层。

### 8.2 代码组织

- **MyZero 是项目唯一的 `*Zero` 实现**，组件全部就地实现于 `src/rl/algo/my_zero/`（v0.25 Phase 1，自包含）：value_encoding / value_transform / n_step / reanalyze / loss / consistency / value_prefix / target_net / sve。
- **旧 `muzero/` + `efficientzero/` 模块与示例已删除**（组件已被 my_zero 吸收）；其论文出处见 [MyZero 总览](../../examples/my_zero/README.md) 脚注（学术溯源，与代码依赖无关）。
- SAC（`algo/sac/`）和 PPO（`algo/ppo/`）不受影响，继续作为 model-free 基线独立存在。

### 8.3 第一步：CartPole-v1 base + 消融序列

1. **base**：最简 learned-model MCTS（= MuZero canonical，无 EZ 增量），CartPole-v1 greedy eval ≥ 475
2. **+consistency**：自监督 SimSiam，消融前后对比样本效率（仅 consistency ~29k env-steps）
3. **+reconstruction**：Scholz et al. 2021 自监督重建；与 consistency 联用 CartPole ✅ ~**11.7k** env-steps（**当前 promoted 配方**）
4. **+value prefix**：LSTM 累计 reward 前缀（含 hidden 穿树），消融（CartPole ❌ 有害）
5. **+target net**：EMA/hard 同步，消融
6. **+SVE**：search value 修正 stale target，消融（现固定权重；自适应 mixed target 属 Phase 2 改进，见 [算法纲领 §5.1](./my_zero_algorithm_vision.md)）
7. **+Gumbel**：序贯减半连续搜索（Pendulum 起才真正需要）

> **CartPole 当前内置（2026-06-21）**：base + **consistency + reconstruction**；reanalyze 写回已实现但 recipe 暂不开启（[issue](../../.issue/items/my_zero_reanalyze_cartpole_regression.md)）。

### 8.4 改动纪律（搬运 ≠ 改进）

- **搬运**（挪纯函数、改 import、折 config、删旧 `muzero/`/`efficientzero/` 残留）：行为零变化 → 可批量；**CartPole-v1 greedy eval ≥ 475** 为回归哨兵。
- **改进**（stop-grad / SVE 自适应 / value 上游 target 等）：改行为 → **一次一项**，单独 A/B 消融，单独过哨兵；**不得与搬运混进同一改动**。
- Agent 改 RL 代码时见 [`.github/instructions/rl.instructions.md`](../../.github/instructions/rl.instructions.md)。
