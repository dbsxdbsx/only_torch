...
│  ├── RNNCell, LSTMCell, GRUCell, ∆-RNNCell                  │
│  └── 本质是原子节点的组合，不是新的核心概念                    │
├─────────────────────────────────────────────────────────────┤
│  Level 2: 执行引擎                                           │
│  ├── BPTT 时间调度                                          │
...
> 参考 [高层架构设计](./../_archive/high_level_architecture_design.md)

```
┌─────────────────────────────────────────────────────────────────┐
│ 第5层：底层计算层（Graph/Node/Tensor）                           │
│   职责：单步 DAG 的前向/局部求导                                 │
│   ⚠️ 不应知道"时间"的概念                                        │
├─────────────────────────────────────────────────────────────────┤
│ 第4层：中间表示层（ComputationGraph/IR）                         │
│   职责：统一计算图表示                                           │
│   State 节点类型定义在此层                                       │
├─────────────────────────────────────────────────────────────────┤
│ 第3层：执行引擎层（ExecutionEngine）                             │
│   职责：BPTT 时间调度、跨时间梯度移位、TBPTT 截断               │
│   BPTT 的"展开/梯度传递"逻辑在此层                              │
├─────────────────────────────────────────────────────────────────┤
│ 第2层：演化API层（NEAT）                                         │
│   职责：结构变异、物种形成、recurrent_depth 配置                │
├─────────────────────────────────────────────────────────────────┤
│ 第1层：用户API层（Module/Optimizer/DataLoader）                  │
│   用户无感知时间调度细节                                         │
└─────────────────────────────────────────────────────────────────┘
```

**关键洞察**：BPTT 不是"把单步 backward 调 3 次"，而是**执行器层面的时间调度问题**。
