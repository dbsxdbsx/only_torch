...
## 四c、每局耗时增长定性 + u8 量化帧存储（2026-07-03，已闭环）

「25s→68s 平台」归因锤死为**工作集 × 随机读**（f32 帧 buffer ≈ 800MB，随机抽样每读必冷，
内存带宽瓶颈；排除 swap 与「agent 变强局变长」）。实施 `StoredObs` u8 量化帧存储
（buffer 800MB→200MB）：**逐局 wall Ep5~Ep60 全程平坦 ~10s，增长斜率归零**（平台期
对比 ~7×、健康环境同期对比 ~3×），60 局零崩溃（Ep53 被动陷阱干净通过）。量化口径
（round，误差 ≤ 0.5/255）与数据见[优化战报 M](../performance/optimization_log.md)与
[pong README 工程基线](../../examples/my_zero/pong/README.md)。150 局/seed 成本估算
从 ~3h 降至 **~20 分钟**——S2 基准与 S3 三臂消融在本机变为工程可行。

## 五、待办

- [x] ~~Ep53 崩溃根因~~ → 未能复现，转被动监视（[issue](../../.issue/items/pong_ep53_panic_investigation.md)）；u8 版 60 局再次干净通过
- [x] ~~每局耗时随 buffer 占用增长的定性~~ → 已闭环（§四c，u8 量化帧存储）
- [ ] 3-seed 基准数字 + 学习曲线 → 账本（u8 后 ~20min/seed，可过夜或午休挂）
- [ ] 三臂消融 → 账本
- [ ] 文档同步(roadmap/AGENTS/CHANGELOG)——**全部改动不提交,待用户检查**

> 曾议而不立：「分钟级图像哨兵环境（Catch 类）」——u8 存储后 Pong 单 seed 已降至
> ~20min，中间阶梯的必要性消失，图像线继续以 Pong 为唯一载体（2026-07-03 用户裁决）。
