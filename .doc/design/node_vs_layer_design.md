...
| **损失** | `MSE`, `MAE`, `BCE`, `Huber`, `SoftmaxCrossEntropy` | 损失计算（IoU 族 bbox loss 由 `vision::detection::iou_loss` 用基础算子拼接，不在节点层） |
...
| **CNN** | `[batch, C, H, W]` | 必须 4D，batch 在第一维 |

**关键特性**：
- **单样本是 batch_size=1 的特例**：FC 层用 `[1, features]`，CNN 用 `[1, C, H, W]`
- **Batch 维度始终在第一位**
