...
|------|------|---------------|------|------|
| **架构跑通** | SAC、MuZero、PPO | **`CartPole-v1`**（满分 500，solved = **475**） | 发版 / 示例：**greedy(temp=0) eval 20 局均值 ≥ 475** | v0.20 **smoke** 不断言 reward，只验管线。**2026-06-16 全项目统一 v1、废弃 v0**（v0 仅 DeprecationWarning + v1 是 EZ 对标基准）；MuZero greedy 噪声大、首达可能 spike |
| **终极调优** | **[EfficientZero V2](https://arxiv.org/abs/2403.00564)（EZ-V2，第二代，唯一）** | **`-v1` / 新版 ID** | 各任务专属指标（侧重样本效率 env-steps） | 离散+连续+视觉+低维；混合 Tuple 为工程扩展 |
...
- **一个算法，持续迭代**：不再为每篇论文建独立目录（`muzero/` / `efficientzero/`），而是维护一个不断进化的 MyZero。
...
- **组件公共化**：消融过程中发现可复用的组件（如 categorical support、value transform、n-step target），从 `algo/muzero/` 提取到更公共的位置（如 `algo/common/` 或 `mcts/` 层），供 MyZero 和旧代码共用。

### 8.2 与旧代码的关系

- `src/rl/algo/muzero/`、`src/rl/algo/efficientzero/`、`examples/muzero/`、`examples/efficientzero/`：**保留为消融对照基线**。MyZero 迭代过程中需要 A/B 对比。
- 全动作/状态类型覆盖且 MyZero 达标后，旧算法代码**整体删除**，MyZero 成为唯一 `*Zero` 实现。
