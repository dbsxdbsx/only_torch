...
2. **+consistency**：自监督 SimSiam，消融前后对比样本效率
3. **+value prefix**：LSTM 累计 reward 前缀（含 hidden 穿树），消融
4. **+target net**：EMA/hard 同步，消融
5. **+SVE**：search value 修正 stale target，消融
6. **+Gumbel**：序贯减半连续搜索（Pendulum 起才真正需要）
