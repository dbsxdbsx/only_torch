...
| Detection | 多目标 2D Detection | `[~]` | 已沉淀通用 `BBox` 坐标契约、IoU family、带 score/top-k/max-det 的 NMS、矩形 letterbox、mAP / precision / recall / F1、per-class AP、YOLO txt parser、label / prediction 双侧 letterbox 同步反映射（label 侧 `letterbox_labels` / `restore_letterbox_labels`，prediction 侧 `Detection::map_to_origin` 单框 + `restore_letterbox_detections` 批量）、`DetectionBatch`、通用 detection loss 组合与拼接式 `bbox_loss(IoU/GIoU/DIoU/CIoU)`（基础算子 + autograd 自动反向，与 PyTorch 在 `epsilon ≤ 1e-5` 内逐元素一致，可视化层折叠成单个 IoULoss/GIoULoss/DIoULoss/CIoULoss cluster）；已立 `Backbone` / `DetectionHeadDecode` / `Assigner` 类型契约（`vision/detection/contract.rs`）；`chinese_chess_yolov5_onnx_recognize_fen` 可导入第三方 YOLOv5 ONNX 做推理演示 | only_torch 原生检测 head、assignment / matching、端到端 fine-tune 示例尚未支持 |
...
| Unified | 多头视觉模型 | `[~]` | 传统 API 已有多输入 / 多输出示例；Evolution 已支持固定数量命名 head 的平坦 supervised 多头任务、逐 head loss / metric 聚合与选择性推理；detection 已具备变长实例、NMS 与 mAP 基础积木 | 空间 / 序列多头、检测实例 matching 与多任务训练闭环尚未支持 |
...
| YOLO-lite Detection 前置能力 | `[~]` | 通用 bbox 几何、后处理、指标、检测 batch、同步标签变换、detection loss 组合与任务契约（`Backbone` / `DetectionHeadDecode` / `Assigner`）已入库；下一步实现 grid head、assignment / matching 与可训练 example，这是 only_torch 原生进入多目标检测的最小可行路径 |
