...
| PerceptionLoss | 标量 `[1,1]` | ⚠️ 待迁移 | ⚠️ 待迁移 |
...
**1.1 实现新 API**
- [ ] 实现统一的 `graph.backward(loss)`（内部调用现有 VJP 逻辑）
- [ ] 实现统一的 `graph.zero_grad()`

**1.2 旧 API 转发（关键步骤）**
- [ ] 将 `backward_nodes()` 内部改为转发到新的 VJP 实现
- [ ] 将 `backward_batch()` 内部改为转发到新的 VJP 实现
...
### Phase 2：清理 Jacobian 代码（1-2 周）

> 前置条件：Phase 1.2 完成后，旧 API 已不再依赖 Jacobian，可安全删除

- [ ] 删除 `TraitNode` 中的 `jacobi` 相关方法
- [ ] 删除所有节点结构体的 `jacobi` 字段
- [ ] 删除 `calc_jacobi_to_a_parent` 实现
- [ ] 更新 `on_topology_changed()` 清理 `grad` 而非 `jacobi`
- [ ] 确保所有操作节点的单元测试通过

### Phase 3：统一优化器（1 周）

- [ ] 实现统一的 `optimizer.step()`
- [ ] 将 `one_step()` / `one_step_batch()` 内部改为转发到新实现
- [ ] 标记旧优化器方法为 `#[deprecated]`
- [ ] 更新优化器相关单元测试
...
**4.2 文档更新**
- [ ] 更新 README 和 API 文档
- [ ] 编写迁移指南，引导用户从旧 API 迁移到新 API

### Phase 5：彻底删除废弃代码（下个大版本）

- [ ] 删除所有 `#[deprecated]` 标记的旧 API
- [ ] 删除 `jacobi` 相关的所有残留代码
- [ ] 清理测试中对旧 API 的引用
