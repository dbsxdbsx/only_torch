...
> **2026-06-16 更新**：全项目 CartPole 统一到 **v1**，废弃 v0（v0 在新 Gymnasium 仅 DeprecationWarning，且 v1 是 EZ 对标基准）。

| 用途 | 环境 ID | 说明 |
|------|---------|------|
| **SAC / MuZero / PPO 架构跑通** | **`CartPole-v1`** | 满分 500；Gym solved = **greedy(temp=0) eval 20 局均值 ≥ 475**（2026-06-16 统一 v1、废弃 v0） |
| **EfficientZero V2（EZ-V2）终极调优** | **`CartPole-v1`** | 满分 500；侧重样本效率（达天花板所需 env-steps），分数天花板 500 已达 |
...
**CartPole-v0 存续状态**：Gymnasium 1.3.0 main 分支仍注册（`max_episode_steps=200, reward_threshold=195.0`），每次 `make` 会打 DeprecationWarning 建议升 v1。翻遍 v0.26–v1.3.0 全部 release notes **无任何移除计划**；被标记 deprecated + 计划移除的只有 MuJoCo v2/v3（基于老 mujoco-py）。v0.20–v0.23 曾用 v0 做架构跑通标准（200 步 truncation 教学价值）。**2026-06-16 更新**：v0.24 起**全项目 CartPole 统一到 v1**（含 SAC/MuZero/PPO 架构跑通），**不再使用 v0**——正因 v0 已被 DeprecationWarning 标记 + v1 是 EZ-V2 对标基准；架构跑通门槛随之改为 greedy eval ≥ 475。
...
|:----:|---------|---------|---------|---------|---------|
| 1 | `pip install gymnasium` | CartPole-v1 | Box(4,) | Discrete(2) | SAC/MZ/PPO 架构跑通（greedy≥475）+ EfficientZero V2（样本效率）；2026-06-16 统一 v1 |
