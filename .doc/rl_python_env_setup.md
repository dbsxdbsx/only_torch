...
> **2026-06-16 更新**：全项目 CartPole 统一到 **v1**，废弃 v0（v0 在新 Gymnasium 仅 DeprecationWarning，且 v1 是 MyZero 对标基准）。

| 用途 | 环境 ID | 说明 |
|------|---------|------|
| **SAC / MyZero / PPO 架构跑通** | **`CartPole-v1`** | 满分 500；Gym solved = **greedy(temp=0) eval 均值 ≥ 475**（2026-06-16 统一 v1、废弃 v0） |
| **MyZero 终极调优** | **`CartPole-v1`** | 满分 500；侧重样本效率（达天花板所需 env-steps），分数天花板 500 已达 |
...
|:----:|---------|---------|---------|---------|---------|
| 1 | `pip install gymnasium` | CartPole-v1 | Box(4,) | Discrete(2) | SAC/MyZero/PPO 架构跑通（greedy≥475）+ MyZero 样本效率；2026-06-16 统一 v1 |
