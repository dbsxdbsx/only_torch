...
| 项 | 内容 | 触发条件 |
|---|---|---|
| **并行 self-play** | 每核一局、各持网络副本（权重常驻各核私有 L2，不抢 L3/带宽，预期近线性扩展）；AlphaZero 系标准架构的 CPU 多核版。⚠️ 改变数据新鲜度语义（对弈权重比训练线程略旧），须预注册 A/B。**便宜前置档（2026-07-07 注记，用户考虑中）：seed 级进程并行**——3-seed 臂的各 seed 完全独立（各自 RNG 流，结果逐 bit 不变），预编译二进制 + seed 环境变量分进程跑即可 ÷3 臂墙钟（2.5h→50min），零算法语义变化、无链接器锁（只执行不重链）；实测背景 = ⑫⑬ 臂 5000 局时代单进程仅用 32 线程中的 3–4 个（11% CPU，i9-13900HX 24 核） | **触发条件已满足（2026-07-05/07）**：⑫⑬ 臂 40–55 min/seed、2.5–3.5h/臂（写下本条时 Gomoku ~90s/seed 可忍的前提已失效） |
