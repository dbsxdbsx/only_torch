...
1. **内核级优化路线整体否决**（Winograd / GEMM 布局传播 / 三值内核 / 快速矩阵乘）——我们的 GEMM 刻意外包给 `matrixmultiply` / MKL，这条线若某天实用化会经由上游 BLAS 库免费继承，不需要也不应该自己实现。详见 [optimization_candidates.md 已否决项](../optimization_candidates.md#已否决项)。
...
|---|---|---|---|---|---|
| 2509.26217 | CPU 卷积算法能耗横评（OneDNN 实测 direct/im2row/gemm/wino） | A | ✅ 证据归档 | 单算子 wino 最快，但**整网推理数据搬运反噬，隐式 GEMM 全面胜出**；1×1 快路径与 batch=1 场景假设与我们吻合 | Winograd 盖棺 → [已否决项](../optimization_candidates.md#已否决项)；隐式 lowering 记为图像线唯一潜在方向 |
...
| 2308.13898 | 复杂拓扑 DNN 内存感知调度（ILP + 最优性保持融合） | B | ❌ 否决·留一思想 | 解决"边缘设备峰值内存放不下"，我们桌面 CPU 无此问题；其穷人版思想（推理期按 liveness 及早释放中间节点 value，~十几行）值得留档 | [待优化项 #3](../optimization_candidates.md#待优化项) |
