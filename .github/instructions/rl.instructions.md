...
- **验收分层**：SAC / MuZero / PPO 架构跑通用 **`CartPole-v1` 且 greedy(temp=0) eval 20 局均值 ≥ 475**（Gymnasium 官方 solved；**2026-06-16 全项目 CartPole 统一 v1、废弃 v0**——v0 在新 Gymnasium 仅 DeprecationWarning，且 v1 是 EZ 对标基准）；**[EfficientZero V2（EZ-V2，第二代）](https://arxiv.org/abs/2403.00564)** 为唯一终极调优算法（**不以 V1 为发版目标**），全部示例环境用 **`-v1` / 新版 ID**，侧重样本效率（env-steps）（见路线图 §5.6.1、§5.8）。
