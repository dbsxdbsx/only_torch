...
- 改动前先看 [RL 状态总览](../../.doc/design/rl_myzero_status.md)（当前态 + 验收协议 + 待办）与 [Python 环境配置](../../.doc/rl_python_env_setup.md)。
...
- **benchmark 数字按环境分账**：[CartPole](../../examples/my_zero/cartpole/README.md) / [Gomoku](../../examples/my_zero/gomoku/README.md)——改数字只改对应 owner（带口径列），其余文档一律链入不复制；「变慢 ≠ 失败」，只有不收敛/不达标才记 known-fail issue。
- **MyZero 改动纪律（搬运 ≠ 改进）**：
  ...
  - 战略裁决见 [RL 状态总览 §7](../../.doc/design/rl_myzero_status.md#7-关键设计决策备忘)；组件×环境裁决矩阵见 [MyZero 示例总览](../../examples/my_zero/README.md)。
