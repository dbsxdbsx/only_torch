...
print("\n=== Rust测试用数据 ===")
print(f"// 标量值")
print(f"let scalar_data = &[2.0];")
print(f"// 矩阵值")
print(f"let matrix_data = &[{', '.join(map(str, matrix.detach().numpy().flatten()))}];")
print(f"// 预期输出")
print(f"let expected_output = &[{', '.join(map(str, result.detach().numpy().flatten()))}];")
print(f"// 对标量的雅可比矩阵")
print(f"let expected_jacobi_scalar = &[{', '.join(map(str, jacobi_to_scalar.flatten()))}];")
print(f"// 对矩阵的雅可比矩阵")
print(f"let expected_jacobi_matrix = &[{', '.join(map(str, jacobi_to_matrix.flatten()))}];")
