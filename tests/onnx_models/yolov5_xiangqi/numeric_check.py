...
# 当前状态
only_torch 的 import + rebuild 路径已能通过 VinXiangQi YOLOv5 回归门。
本脚本只负责生成 onnxruntime 的固定输入/输出参考；Rust 端 ignored
数值对照测试会读取这些 .npy，与 only_torch raw output 做统计对比。
...
def main() -> None:
    ...
    print()
    print("  完成。Rust 端 forward 数值对照步骤：")
    print(
        "    1. cargo test --test yolov5_xiangqi_import forward_numerical_match_with_onnxruntime -- --ignored --nocapture"
    )
    print(
        "    2. 默认只打印统计；设置 ONLY_TORCH_STRICT_YOLO_NUMERIC=1 可启用严格断言"
    )
