# -*- coding: utf-8 -*-
"""
Calculate expected values for HardSigmoid node forward and Jacobian
For Rust unit test verification
hardsigmoid(x) = 0 if x<=-3, 1 if x>=3, (x+3)/6 otherwise
"""
...
print("=== HardSigmoid Node Test ===")
print(f"Input x:\n{x.detach().numpy()}")

y = torch.nn.functional.hardsigmoid(x)
print(f"\nForward hardsigmoid(x):\n{y.detach().numpy()}")

n = x.numel()
jacobi = np.zeros((n, n))
for i in range(n):
    x_clone = x.clone().detach().requires_grad_(True)
    y_clone = torch.nn.functional.hardsigmoid(x_clone)
