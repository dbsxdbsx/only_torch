"""
Tensor Mul (*) Broadcast Reference Test

Test cases for element-wise multiplication with broadcasting.
Broadcasting rules are identical to addition.
...
def test_broadcast_mul(shape_a, shape_b):
    """
...
def main():
    ...
    test_cases = [
        ...
        ((3, 4), (4,), "[batch,out] * [out] (mask/scale)"),
    ...
    for shape_a, shape_b, desc in test_cases:
        result = test_broadcast_mul(shape_a, shape_b)
...
///
/// Reference: tests/python/tensor_reference/tensor_mul_broadcast_reference.py
#[test]
fn test_mul_broadcast() {
    ...
    for (shape_a, data_a, shape_b, data_b, expected_shape, expected_data) in test_cases {
        ...
        let result_swap = &b * &a;
        assert_eq!(result_swap, expected, "* commutative failed: {:?} * {:?}", shape_b, shape_a);
    }
}""")
