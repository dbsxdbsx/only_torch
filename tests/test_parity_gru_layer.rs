/*
 * IT-3d: 奇偶性检测集成测试（变长 + Batch + GRU Layer API）
 *
 * 与 IT-3b/IT-3c 相同的任务，但使用 GRU Layer API
 * 目的：验证 GRU Layer API 在真实任务上的可用性
 *
 * 网络结构：
 *   input_t [B, 1] ──→ [GRU Layer] ──→ hidden_t [B, H]
 *                           │               │
 *   mask_t [B, H] ──→ [状态冻结] ──→ masked_h [B, H]
 *                                           │
 *                    [Linear: W_ho] ──→ [Sigmoid] ──→ output [B, 1]
 *
 * GRU 相比 LSTM 结构更简单（2 门 vs 4 门），计算效率更高
 *
 * 验收标准：
 *   1. 使用 gru() 层 API 正确构建网络
 *   2. 变长序列能正常训练
 *   3. 达到 90% 以上测试准确率
 */

use only_torch::nn::layer::gru;
...
/// 使用 GRU Layer API 创建网络
fn create_parity_gru_with_layer_api(
    batch_size: usize,
    hidden_size: usize,
    seed: u64,
) -> Result<
...
> {
    ...
    // === 使用 GRU Layer API ===
    let gru_out = gru(&mut graph, input, 1, hidden_size, batch_size, Some("gru"))?;

    // 手动初始化 GRU 权重（使用 Xavier 风格）
    let mut hasher = DefaultHasher::new();
    ...
    // === 状态冻结逻辑 ===
    let neg_one = graph.new_parameter_node(&[1, 1], Some("neg_one_mask"))?;
    graph.set_node_value(neg_one, Some(&Tensor::new(&[-1.0], &[1, 1])))?;

    // masked_h = h_prev + mask * (hidden - h_prev)
    let neg_h_prev = graph.new_scalar_multiply_node(neg_one, gru_out.h_prev, Some("neg_h_prev"))?;
    let delta = graph.new_add_node(&[gru_out.hidden, neg_h_prev], Some("delta"))?;
    let masked_delta = graph.new_multiply_node(mask, delta, Some("masked_delta"))?;
    let masked_h = graph.new_add_node(&[gru_out.h_prev, masked_delta], Some("masked_h"))?;
...
/// IT-3d: 使用 GRU Layer API 的变长奇偶性检测
#[test]
fn test_parity_detection_with_gru_layer() {
    println!("\n========== IT-3d: 变长奇偶性检测（GRU Layer API）==========\n");

    // 超参数（GRU 相比 LSTM 更简单，收敛更快）
    let batch_size = 16;
    let min_len = 3;
    let max_len = 8;
    let num_train = 512;
    ...
    // 创建网络（使用 GRU Layer API）
    let (mut graph, input, mask, output, loss, target, params) =
        create_parity_gru_with_layer_api(batch_size, hidden_size, seed).expect("创建网络失败");

    println!(
        "\n[网络] 使用 gru() Layer API, {} 个可训练参数, hidden_size={}",
        params.len(),
        hidden_size
    );

    // 可视化（启用层分组）
    match graph.save_visualization_grouped("tests/outputs/it3d_parity_gru_layer", None) {
        Ok(out) => {
    ...
    println!(
        "\n✅ IT-3d 通过！GRU Layer API 达到 {:.1}% 准确率\n",
        best_acc * 100.0
    );
}
