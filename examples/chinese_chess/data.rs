//! 中国象棋训练数据加载器
//!
//! 读取 `scripts/generate_chess_data.py` 生成的二进制格式数据：
...
/// 加载二进制数据
///
/// 返回 (images_tensor, labels_tensor, num_classes)
/// - images: [N, C, H, W] float32
/// - labels: [N, num_classes] one-hot float32
pub fn load_chess_data(data_dir: &str) -> Result<(Tensor, Tensor), String> {
    let images_path = Path::new(data_dir).join("images.bin");
    let labels_path = Path::new(data_dir).join("labels.bin");
    ...
    // 读取图像
    let mut img_file = File::open(&images_path)
        .map_err(|e| format!("打开 images.bin 失败: {e}"))?;

    let mut header = [0u8; 16];
    img_file.read_exact(&mut header)
    ...
    img_file.read_exact(&mut img_bytes)
    ...
    // 读取标签
    let mut lbl_file = File::open(&labels_path)
        .map_err(|e| format!("打开 labels.bin 失败: {e}"))?;

    let mut lbl_header = [0u8; 4];
    lbl_file.read_exact(&mut lbl_header)
    ...
    lbl_file.read_exact(&mut label_bytes)
