//! MyZero CartPole 模型定义（categorical value/reward + latent min-max 归一化）
//!
//! 三网络架构：
...
use only_torch::rl::algo::efficientzero::negative_cosine_similarity;
...
// ============================================================================
// Value Prefix LSTM（S2：用 LSTM 预测累计 reward 前缀和）
// ============================================================================

/// LSTM cell + prefix head，用于 value prefix 消融（S2）。
///
...
// ============================================================================
// Projector / Predictor 网络（S1 consistency 专用，SimSiam 分支）
// ============================================================================
...
pub struct MyZeroModel {
    ...
    pub lstm: ValuePrefixLstm, // S2 value prefix LSTM
    pub graph: Graph,
...
impl MyZeroModel {
    pub fn new(
        ...
        let lstm_hidden = latent_dim; // LSTM hidden 维度 = latent_dim（64）
        Ok(Self {
    ...
    pub fn train_unroll(
        ...
        // S2 value prefix：LSTM hidden state 初始化为全零
        ...
        for i in 0..k {
            ...
            // reward loss：S2 开启时用 LSTM prefix logits，否则走原 DynamicsNet reward head
            ...
            // S1 consistency：dynamics 预测的 next_latent 与 repr 编码的真实 next_obs 对齐
