...
> 支持：`EZ_CONS / EZ_VP / EZ_TARGET / EZ_SVE / CQ / CQ_SCALE / CQ_VISIT / SIMS / SEEDS / SMOKE / GAMMA / MAX_EP / LR / NUM_ACTIONS / RSCALE`
...
| reward head 分辨率（RSCALE=0.1）| ⚠️ **头号嫌疑** | 单步 reward 压到 `h=−0.83`，41 原子里只占中心 0.8 格；RSCALE 0.1→1.0 全程不溢出，可把有效原子 1.6→7.6 格（5×）|
| sims 不够（9 动作 × 50 sims）| ⏸ **硬约束不碰** | MuZero/EZ 上限就是 50 sims；用户领域知识：不靠加算力解决 |
| EZ_CONS 实现（同 projector + 无 stop-grad）| ⚠️ **环境敏感** | 偏离 SimSiam 设计有塌缩风险，但 CartPole 开 cons 反而学得好（loss 9.6→0.7）→ 非致命 bug，须 base 对照定性 |
...
| C: +consistency | `EZ_CONS=1` | 隔离 cons 真实作用 | 未跑 | — |
