...
fn main() -> Result<(), GraphError> {
    ...
    let model = ParityGRU::new(&graph, hidden_size)?;
...
fn evaluate(model: &ParityGRU, test_loader: &BucketedDataLoader<'_>) -> Result<f32, GraphError> {
    let mut correct = 0;
    let mut total = 0;
