 ...
 * 框架会按图像分类任务自动启用已验证的搜索策略，用户侧不需要配置模型结构。
 *
 ...
 * - FlatMLP / TinyCNN / LeNetTiny 初始候选族可进入同一套训练评估流程
 ...
 * ## 性能参考（debug 模式）
 * - 5000 训练样本 + 1000 测试样本
 * - 目标 95% 准确率；debug + BLAS 下通常 1 分钟内完成
 */
...
fn env_seed() -> u64 {
    env::var("ONLY_TORCH_MNIST_SEED")
        .ok()
        .and_then(|v| v.parse().ok())
        .unwrap_or(DEFAULT_SEED)
}

fn save_artifacts_enabled() -> bool {
    env::var("ONLY_TORCH_MNIST_SAVE_ARTIFACTS")
...
fn main() {
    ...
    let seed = env_seed();
    let save_artifacts = save_artifacts_enabled();
    ...
    // 2. 准备数据子集
    let train_data = collect_samples(&train_dataset, TRAIN_SAMPLES, seed);
    let test_data = collect_samples(&test_dataset, TEST_SAMPLES, seed + 1);

    println!("\n[2/3] 配置：");
    println!("  - 训练样本: {TRAIN_SAMPLES}（mini-batch, batch_size=128）");
    println!("  - 测试样本: {TEST_SAMPLES}");
    println!("  - 输入: [1, 28, 28]（灰度图）");
    println!("  - 输出: 10 类（数字 0-9）");
    println!("  - 搜索策略: Evolution 自动选择空间分类默认策略");
    println!("  - seed: {seed}");
    ...
    if save_artifacts {
        ...
        println!("已跳过可视化和模型保存/加载（ONLY_TORCH_MNIST_SAVE_ARTIFACTS=0）");
    }
