...
$$
\text{Gaussian } [-1,1]：H ∈ (-∞, +∞) \quad → \quad target = -d \text{（经验值，无天然锚点）}
$$

$$
\text{Beta } [-1,1]：H ∈ (-∞, ln(2)] \quad → \quad target = c × ln(2) × d \text{（有天然锚点）}
$$
...
- Beta 的有界熵对 Hybrid SAC 有额外好处——与离散部分的尺度更匹配，详见 [数学基础分析](sac_mathematical_foundations.md#6-beta-分布的理论优势)

## 相关文档

- [SAC 示例总览](README.md) — 熵、α、Target Entropy 的核心概念
- [SAC 数学基础分析](sac_mathematical_foundations.md) — 收敛性、KL 散度、测度论等理论深度分析
