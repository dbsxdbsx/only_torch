...
$$
H_d^{max} ≈ 1.1 \quad \left(= ln(3)\right)
$$

$$
H_c \text{ 典型值} ≈ 14.2 \quad \left(10 \text{ 维 Gaussian，} σ ≈ 1\right)
$$
...
$$
= H_{Shannon}(ad) + E_{ad}[H_{differential}(ac|ad)] \quad \leftarrow \text{链式法则}
$$
...
$$
log π(a|s) = log p(z₀) - Σ_{k=1..K} log|det(∂f_k/∂z_{k-1})| - Σ log(1 - tanh²(zK_i))
$$

（依次为：基础 Gaussian、中间 flow 层 Jacobian、最终 tanh 层 Jacobian）
...
- [RL 状态总览](../../.doc/design/rl_myzero_status.md) — RL 架构设计与版本规划
