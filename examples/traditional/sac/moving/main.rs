...
fn main() -> Result<(), GraphError> {
    ...
    Python::attach(|py| {
        ...
        println!(
            "  Actor: {obs_dim} → {} → {} → [离散(3), Acc(1), Turn(1)]",
        ...
        for episode in 0..config.max_episodes {
            ...
            loop {
                ...
                let (discrete_action, action_vec) = agent.actor.select_action(&obs_tensor)?;
                    ...
                    && total_steps.is_multiple_of(config.update_every)
                {
                    ...
                    let next_actor_out = agent.actor.forward_all_branches(&next_obs_batch)?;
                    ...
                    let next_turn_action = next_actor_out.turn_env_action.value()?.unwrap();
                    ...
                    let cont_for_acc = Tensor::concat(&[&next_acc_action, &zeros_bs1], 1);
                    let tq1_acc = agent
                        .target_critic1
                        .get_q_values(&next_obs_batch, &cont_for_acc)?;
                    let tq2_acc = agent
                        .target_critic2
                        .get_q_values(&next_obs_batch, &cont_for_acc)?;
                    ...
                    // Turn: cont = [0, rot]
                    let cont_for_turn = Tensor::concat(&[&zeros_bs1, &next_turn_action], 1);
                    let tq1_turn = agent
                        .target_critic1
                        .get_q_values(&next_obs_batch, &cont_for_turn)?;
                    let tq2_turn = agent
                        .target_critic2
                        .get_q_values(&next_obs_batch, &cont_for_turn)?;
                    ...
                    let next_log_prob_c =
                        Tensor::concat(&[&next_acc_lp, &next_turn_lp, &zero_lp], 1);
                    // 构造 min_Q: [bs, 3]
                    let next_min_q = Tensor::concat(&[&tq_acc_col, &tq_turn_col, &tq_brake_col], 1);
                    ...
                    let actor_out = agent.actor.forward_all_branches(&obs_var_actor)?;
                    ...
                    let cont_acc_var = Var::concat(&[&actor_out.acc_env_action, &zeros_var], 1)?;
                    ...
                    let q_min_acc = (&q1_acc + &q2_acc - q_diff_acc.abs()) * &half;

                    // Turn: [0, turn_action]
                    let cont_turn_var = Var::concat(&[&zeros_var, &actor_out.turn_env_action], 1)?;
                    let q1_turn = agent.critic1.forward_q(&obs_var_actor, &cont_turn_var)?;
                    let q2_turn = agent.critic2.forward_q(&obs_var_actor, &cont_turn_var)?;
                    let q_diff_turn = &q1_turn - &q2_turn;
                    let q_min_turn = (&q1_turn + &q2_turn - q_diff_turn.abs()) * &half;

                    // Brake: [0, 0]
                    let cont_brake_var = Var::concat(&[&zeros_var, &zeros_var], 1)?;
                    let q1_brake = agent.critic1.forward_q(&obs_var_actor, &cont_brake_var)?;
                    let q2_brake = agent.critic2.forward_q(&obs_var_actor, &cont_brake_var)?;
                    let q_diff_brake = &q1_brake - &q2_brake;
                    let q_min_brake = (&q1_brake + &q2_brake - q_diff_brake.abs()) * &half;
                    ...
                    // min_Q: [bs, 3]
                    let min_q_var = Var::concat(&[&q_acc_col0, &q_turn_col1, &q_brake_col2], 1)?;

                    // log_prob_c: [bs, 3]（Brake 列 = 0 哑值）
                    let zero_lp_var = Tensor::new(&vec![0.0; bs], &[bs, 1]).into_var(&graph)?;
                    ...
                    let actor_loss = (&actor_out.probs
                        * (&actor_out.log_probs * &alpha_d_t + &log_prob_c_var * &alpha_c_t
                            - &min_q_var))
                        .sum_axis(1)
                        .mean();
            ...
            if episode_rewards.len() >= 50 {
                let recent: f32 = episode_rewards.iter().rev().take(50).sum::<f32>() / 50.0;
                if recent >= config.target_reward {
                    println!(
                        "\n  达到目标！近 50 ep 平均 R={:.3}（目标 >= {:.1}）",
                        recent, config.target_reward
                    );
                    solved = true;
                    break;
                }
        ...
        // 7. 保存计算图可视化
        println!("\n[5/6] 保存计算图可视化...");
        let vis_result = graph.visualize_snapshot("examples/traditional/sac/moving/moving_sac")?;
        ...
        if solved || avg_test_reward >= config.target_reward {
            println!(
                "  Moving-v0 Hybrid SAC（方式 B）示例成功！（测试平均 R={avg_test_reward:.3}）"
            );
        } else {
            println!(
