...
//!
//! 数据格式（由 `examples/traditional/chinese_chess_cnn_onnx_finetune/generate_data.py` 生成）：
...
fn load_split(split_dir: &Path) -> Result<(Tensor, Tensor), String> {
    ...
    if !images_path.exists() {
        return Err(format!(
            "数据文件不存在: {}\n请先运行: python examples/traditional/chinese_chess_cnn_onnx_finetune/generate_data.py",
            images_path.display()
        ));
    }
