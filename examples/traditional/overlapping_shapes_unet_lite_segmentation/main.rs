//! # Overlapping Shapes U-Net-lite Semantic Segmentation 示例
//!
//! 这个示例复用 64x64 overlapping shapes benchmark，但使用一个小型
//! encoder-decoder + skip connection 的 U-Net-lite 结构，作为后续
//! Segmentation Evolution 扩大 benchmark 时的传统强基线。
//!
//! ## 运行
//! ```bash
...
use model::OverlappingShapesUnetLiteSegmentationNet;
...
use only_torch::vision::mask::{argmax_to_class_map, foreground_from_multiclass};
...
fn main() -> Result<(), GraphError> {
    ...
    println!("=== Overlapping Shapes U-Net-lite Semantic Segmentation 示例 ===\n");
    ...
    let model = OverlappingShapesUnetLiteSegmentationNet::new(&graph)?;
    ...
    println!("网络: U-Net-lite encoder-decoder + skip concat");
    ...
    println!(
        "测试输入图: examples/traditional/overlapping_shapes_unet_lite_segmentation/test_in.png"
    );
    println!(
        "测试输出图: examples/traditional/overlapping_shapes_unet_lite_segmentation/test_out.png"
    );

    let vis_result = graph.visualize_snapshot(
        "examples/traditional/overlapping_shapes_unet_lite_segmentation/overlapping_shapes_unet_lite_segmentation",
    )?;
    ...
    if best_mean_iou >= TARGET_MEAN_IOU {
        println!("Overlapping shapes U-Net-lite semantic segmentation 训练成功。");
        Ok(())
    } else {
        Err(GraphError::ComputationError(format!(
            "Overlapping shapes U-Net-lite semantic segmentation 未达到目标 Mean IoU {:.1}%，最佳 {:.1}%",
...
fn evaluate(
    model: &OverlappingShapesUnetLiteSegmentationNet,
    inputs: &Tensor,
    targets: &Tensor,
    ...
    let foreground_dice = dice_score(
        &foreground_from_multiclass(&probs),
        &foreground_from_multiclass(targets),
        0.5,
    )
    .value();
...
fn save_sample_visualizations(
    ...
    for y in 0..IMAGE_SIZE {
        for x in 0..IMAGE_SIZE {
            ...
            let class_idx = class_map[[sample_idx, y, x]] as usize;
            pixel_block_scale(
                &mut output_img,
                x as u32,
                y as u32,
                palette.color(class_idx),
                OVERLAY_SCALE,
            );
        }
    }
    ...
    save_rgb_image(
        &input_img,
        "examples/traditional/overlapping_shapes_unet_lite_segmentation/test_in.png",
    )
    .map_err(GraphError::ComputationError)?;
    save_rgb_image(
        &output_img,
        "examples/traditional/overlapping_shapes_unet_lite_segmentation/test_out.png",
    )
    .map_err(GraphError::ComputationError)?;
