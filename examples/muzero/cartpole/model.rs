...
//!
//! 三网络架构：
//! - Representation h: obs(4) → latent(64)
//! - Dynamics g: (latent(64), action_onehot(2)) → (next_latent(64), reward(1))
//! - Prediction f: latent(64) → (policy_logits(2), value(1))

use only_torch::nn::{
    Graph, GraphError, IntoVar, Linear, Module, Var, VarActivationOps, VarLossOps,
};
use only_torch::rl::algo::muzero::{loss, value_transform, value_transform_inv};
...
impl DynamicsNet {
    pub fn new(
        ...
        Ok(Self {
            ...
            fc_reward: Linear::new(&graph, 128, 1, true, "fc_reward")?,
        })
    }
...
impl PredictionNet {
    pub fn new(
        ...
        Ok(Self {
            ...
            fc_value: Linear::new(&graph, 128, 1, true, "fc_value")?,
        })
    }
...
impl MuZeroModel {
    ...
    ///
    /// value/reward target 经 `value_transform(h(x))` 后再算 MSE，
    /// 使不同量级的 target 在 loss 中贡献均衡。
    pub fn train_unroll(
        &self,
        obs_t: &[f32],
        actions: &[usize],
        ...
        let tv0 = value_transform(target_values[0]);
        let target_v0 = Tensor::new(&[tv0], &[1, 1]);
        let mut total_loss = pred_policy.cross_entropy(&target_p0)?;
        total_loss = &total_loss + &(&pred_value.mse_loss(&target_v0)? * loss::VALUE_LOSS_COEF);
        ...
        for i in 0..k {
            ...
            let tv = value_transform(target_values[i + 1]);
            let tv_tensor = Tensor::new(&[tv], &[1, 1]);
            let tr = value_transform(target_rewards[i]);
            let tr_tensor = Tensor::new(&[tr], &[1, 1]);

            let step_policy_loss = pred_p.cross_entropy(&tp)?;
            let step_value_loss = pred_v.mse_loss(&tv_tensor)?;
            let step_reward_loss = pred_reward.mse_loss(&tr_tensor)?;
...
impl MuZeroModel {
    fn initial_state_impl(&self, obs: &[f32]) -> (Vec<f32>, Vec<f32>, f32) {
        ...
        let value = value_transform_inv(value_tensor.data_as_slice()[0]).clamp(-300.0, 300.0);
    ...
    fn recurrent_impl(&self, state: &[f32], action: &ActionPayload) -> DynamicsOutput {
        ...
        let reward = value_transform_inv(reward_tensor.data_as_slice()[0]).clamp(-10.0, 10.0);
        let value = value_transform_inv(value_tensor.data_as_slice()[0]).clamp(-300.0, 300.0);

        // MuZero 不使用 terminal head：靠 value prediction 自然学习终局
        // CartPole 最长 200 步，搜索深度受 num_simulations 限制
        DynamicsOutput {
