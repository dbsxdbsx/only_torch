...
| **刚闭环** | **✅ 纯 self-play 万金油战役 ⑫⑬⑭ 三臂收官（2026-07-07）**：① **⑫ 真规则上限标定 ✅ 0.90**——纯无课程 self-play 可行坐实（课程被证为「预算/容量不足下的拐杖」，非必需）；② **⑬ learned dynamics 同配置 ❌ 0.10**——规则学习税坐实 0.8 档，且对 12.5× 预算+CNN+增广**钝感**（墙 = dynamics 学不会「五连终局」，树在幻觉模型里搜索）；③ **⑭ PER 优先回放 ❌ 0.15 带内持平**——消费端排除（PER 重排已有数据，治不了「需要新监督」的病；护栏满分 = 无害留库默认关）；④ **剩余杠杆收敛 = 监督端（recon 重标 / consistency 家族）× 预算量级**；⑤ 工程红利：**seed 级进程并行跑法进库**（`just bench-m3-seedpar`，臂墙钟 ÷3、逐 seed 与串行逐 bit 一致）+ **组件处方表立表**（机制/适应症/禁忌症/剂量四栏，examples/my_zero/README）。前一战役（naive0 战术墙击破 + G1–G3，11 臂 + 课程/KL-lr/领域插件）档案见 CHANGELOG 2026-07-05 与 naive0 issue §二/§四 |
| **当前主线** | **强化学习** v0.26（[路线图 §5](.doc/design/rl_roadmap.md#5-v026-方向2026-07-01-战略转向定稿)）：战略转向「磨观测空间 + self-play」。**已闭环**：P0 loss 系数重标定（recon 1→16 promote）、Phase 0 全项（HL-Gauss / obs symlog 双负结果，CartPole 冻结纯回归哨兵）、哨兵红灯复裁（recipe 零变更回绿，官方 3-seed 中位 ~8.7k env-steps，`.issue/_archive/cartpole_sentinel_red_ndarray_drift.md`）、ROSMO 阶梯一进库（`.rosmo(true)` 默认关，CartPole 闸门 3/3 绿）、**Phase 2 Gomoku 支柱（见「刚闭环」）**。**进行中**：图像线后台预算标定（S2 基准 0/3 平直、S3 组件层排除，嫌疑 = 训练预算差 2 个数量级 + 稀疏 reward，`.issue/items/my_zero_pong_image_flat_negative.md`）；**下一步（⑫⑬⑭ 收官后待与用户复盘定档，收口规划 §4）**：哲学口径维持「纯无课程 self-play × learned dynamics × 通用常驻组件」（⑫ 已证可行、⑬ 证税主导、⑭ 证消费端无解）——候选杠杆 = **监督端**（recon 棋盘重标 {1,4} pilot / consistency 家族给 dynamics 直接监督）× **预算量级加档**（⑬ 证当前量级钝感）× **seed 方差治理 / 多 seed 择优交付**（并行跑法已备好）；并行工程轨 KL-lr 公共化已裁决（CartPole ❌ 灾难不 promote、棋盘无害留库）；后台 = 轻量图像 pilot（MinAtar 类 + 单 seed + 判停，条款三：预算以第三方复现为锚）；**Phase 3 暂缓**（无裁决场）、**Pendulum 复议维持压轴**、**ROSMO 哨兵组件提案已否决**（叠加 29.6k vs 哨兵 ~8.7k）。Pendulum/Platform 已降级非关键路径 |
