...
| **刚闭环** | **✅ Gomoku 棋盘支柱立柱，M0–M4 全程闭环（2026-07-04/05，收口规划 Phase 2 主体完成）**：① 双人 self-play 训练闭环进库 `src/rl/algo/my_zero/board.rs`（negamax 视角轮转 / 根部 legal_mask / negamax MC target，走 `Refreshed` 通道零改动复用 `train_batch`）+ Gumbel 复裁前置双修复（greedy 去噪 + tree-level q_range）；② **M2 预注册双门槛 3/3 达标**（vs random 中位 1.000 / vs 半程快照 gating 0.950，唯一账本 [gomoku README](examples/my_zero/gomoku/README.md)）；③ **M3 九臂消融全中性/弱阳**（Gumbel/cons/recon/CNN/预算×5/replay×8/lr/组合；唯一弱阳性 = D4 增广，未达 promote 线）→ **recipe=base 定型**进 `recipe.rs`、`smoke-rl` 扩容纳入 `smoke-my-zero-gomoku`（8 目标）；④ Gumbel/completedQ 负结果 issue 借 \|A\|≫sims 复裁**终局归档**（棋盘中性、CartPole 灾难归因闭合），三件套分层沉淀 [vision §5.4](.doc/design/my_zero_algorithm_vision.md#54-三件套正交分层sampled--gumbel--completedq2026-07-05-定稿)；⑤ 遗留 **naive0 战术墙**（`.issue/items/gomoku_naive0_tactical_wall.md`，头号假设 MuZero 规则学习税，结构性、不阻塞支柱，后续裁决入口已预注册） |
| **当前主线** | **强化学习** v0.26（[路线图 §5](.doc/design/rl_roadmap.md#5-v026-方向2026-07-01-战略转向定稿)）：战略转向「磨观测空间 + self-play」。**已闭环**：P0 loss 系数重标定（recon 1→16 promote）、Phase 0 全项（HL-Gauss / obs symlog 双负结果，CartPole 冻结纯回归哨兵）、哨兵红灯复裁（recipe 零变更回绿，官方 3-seed 中位 ~8.7k env-steps，`.issue/_archive/cartpole_sentinel_red_ndarray_drift.md`）、ROSMO 阶梯一进库（`.rosmo(true)` 默认关，CartPole 闸门 3/3 绿）、**Phase 2 Gomoku 支柱（见「刚闭环」）**。**进行中**：图像线后台预算标定（S2 基准 0/3 平直、S3 组件层排除，嫌疑 = 训练预算差 2 个数量级 + 稀疏 reward，`.issue/items/my_zero_pong_image_flat_negative.md`）；**下一步（两轨定稿 2026-07-05，收口规划 §4 当前实际次序）**：前台主线 = 五子棋纵深三臂（① sims 400 → ② 树内真规则诊断【若翻墙升级为「棋类松开万金油铁律走真规则树」战略分叉】→ ③ replay32×ROSMO）；后台 = 轻量图像 pilot（MinAtar 类 + 单 seed + 判停，条款三：预算以第三方复现为锚）；**Phase 3 暂缓**（无裁决场）、**Pendulum 复议维持压轴**、**ROSMO 哨兵组件提案已否决**（叠加 29.6k vs 哨兵 ~8.7k）。Pendulum/Platform 已降级非关键路径 |
