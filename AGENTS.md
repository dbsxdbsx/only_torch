...
| **当前主线** | **强化学习** v0.25.0：**MyZero 统一算法**——CartPole 回归哨兵 **consistency + reconstruction · PUCT · sims=20**（~12.2k env-steps）；**completedQ / Gumbel-root 在 CartPole 实测失败**（代码保留、recipe 关，见 `.issue/items/my_zero_gumbel_completedq_cartpole_negative.md`）。Pendulum / 更大动作空间再复测 Gumbel 线 |
