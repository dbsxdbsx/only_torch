...
| **当前主线** | **强化学习** v0.26（[路线图 §5](.doc/design/rl_roadmap.md#5-v026-方向2026-07-01-战略转向定稿)）：战略转向「磨观测空间 + self-play」——**P0 loss 系数重标定已完成（2026-07-02）：recon_coef 1→16 promote，官方哨兵中位 66.2k → ~9.8k env-steps（3/3），超过 bug 时代 13.1k**；**收口规划 Phase 0 已全项闭环（2026-07-02）**：梯度流审计 canonical、HL-Gauss 与 obs symlog 双负结果（开关留库、recipe 零变更、哨兵 ~9.8k 逐 bit 复现），**CartPole 冻结为纯回归哨兵**；**✅ 哨兵红灯已复裁收口（2026-07-04）**：ndarray 漂移红灯（2026-07-03，官方 3-seed 1/3）经最终数值流（战报 P/Q/C1/R 后 HEAD）预注册复裁——recon {4,16} × 5-seed **两臂均 5/5**，按预注册规则 recon=16 维持 promote、**recipe 零变更**，官方 3-seed 哨兵回绿（中位 **~8.7k** env-steps，3/3；PPO 122.9k / SAC 127.8k 同日重测 3/3），哨兵恢复绿灯职责、Phase 1 前置阻断解除（issue 已归档 `.issue/_archive/cartpole_sentinel_red_ndarray_drift.md`，recon=4 range 更紧留作稳健后备档）；同批已落地六项逻辑改良（GroupNorm 梯度断流修复、per-seed env 流独立、温度调度解耦预算等，见 CHANGELOG 2026-07-03）；**Phase 1 图像线诊断收敛 + 次序修订（2026-07-04 晚）**：S0 spike GO ✅ / S1 管线 ✅，但 S2 基准两跑均 0/3 平直（旧/新数值流各一次）、S3 三臂诊断全平（recon pilot {1,4,16} / cons-off / HL-Gauss 均无差异）→ 组件层排除，嫌疑收敛「训练预算差 2 个数量级 + 稀疏 reward」（`.issue/items/my_zero_pong_image_flat_negative.md`）；**图像线降级后台预算标定，Phase 2 Gomoku 提前为当前主线**（[收口规划](.doc/design/rl_closure_plan.md) §2/§3 修订注记：万金油铁律 = 树内纯 learned dynamics、真环境仅走子/根部 legal_mask/终局；base 过预注册门槛后一次一臂，结论只填棋盘列）；**ROSMO 阶梯一已进库（2026-07-04）**：`src/rl/algo/my_zero/rosmo.rs`（一步 look-ahead 改进分布 + 现算 bootstrap + 优势过滤 BC α=0.2，**不写回**），`.rosmo(true)` 消融开关、recipe 默认关，CartPole 回归闸门 **3/3 绿**（中位 29.6k，兼裁旧 reanalyze 灾难 = 机制病理而非实现 bug）。**completedQ / Gumbel-root CartPole 负结果**（代码保留、recipe 关，`.issue/items/my_zero_gumbel_completedq_cartpole_negative.md`）；Pendulum/Platform 已降级非关键路径 |
