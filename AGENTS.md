...
| **刚闭环** | **✅ Gomoku naive0 战术墙击破 + G1–G3 攻坚战役收官（2026-07-05，11 臂预注册消融）**：① **翻墙链条** = 树内真规则（②臂方向性正信号）×**战术开局课程**（`tactical_opening` 构造式生成器进库，Leela 开局库同族机制）——⑦ 终局配方 naive0 中位 0.10→**0.70 翻墙坐实**，双病理（MuZero 规则学习税 × self-play 战术抽签）单治其一不过墙；② **G3 四档验收未达标**（naive0 0.62 <0.75 线、naive1+ 未开），⑧⑨⑪ 把缺口收敛到「**预算量级 ×（可选 CNN）+ seed 方差治理**」（0.17–0.77 六度复现，账本与 issue §四 全档）；③ **KL 自适应 lr 进库**（`kl_adaptive_lr` 开关，⑨臂无害验证，推广待正增益域）；④ **领域插件重构**：`gomoku.rs` 拆出（规则/真规则树/课程三件套，board.rs 保通用管线，象棋照此另立）；⑤ 早间收官：三臂纵深（sims400 排除/rr32×ROSMO 有害反证）+ GPT-5.5 审查（主链路无符号 bug，value_prefix 断链入收口规划 §5） |
| **当前主线** | **强化学习** v0.26（[路线图 §5](.doc/design/rl_roadmap.md#5-v026-方向2026-07-01-战略转向定稿)）：战略转向「磨观测空间 + self-play」。**已闭环**：P0 loss 系数重标定（recon 1→16 promote）、Phase 0 全项（HL-Gauss / obs symlog 双负结果，CartPole 冻结纯回归哨兵）、哨兵红灯复裁（recipe 零变更回绿，官方 3-seed 中位 ~8.7k env-steps，`.issue/_archive/cartpole_sentinel_red_ndarray_drift.md`）、ROSMO 阶梯一进库（`.rosmo(true)` 默认关，CartPole 闸门 3/3 绿）、**Phase 2 Gomoku 支柱（见「刚闭环」）**。**进行中**：图像线后台预算标定（S2 基准 0/3 平直、S3 组件层排除，嫌疑 = 训练预算差 2 个数量级 + 稀疏 reward，`.issue/items/my_zero_pong_image_flat_negative.md`）；**下一步（「纯 self-play 万金油」哲学修正定稿 2026-07-05 晚，收口规划 §4 当前实际次序）**：五子棋验收必须以**纯粹无课程 self-play** 达成，终态 = learned dynamics × 通用常驻组件（课程/真规则树降级诊断脚手架，铁律不破）——前台 = ⑫ 纯 self-play 上限标定（真规则×CNN×5000 局×无课程）→ ⑬ learned dynamics 同配置（⑫⑬ 之差 = 规则学习税定量账）→ ⑭ PER 优先回放（Simulus B1 转正）；并行工程轨 = KL-lr 公共化（CartPole A/B「等效不劣」验收）；终局 = 赢家配方 G3 复训 + 多 seed 择优交付（入口 naive0 issue §四 ⑫⑬⑭）；后台 = 轻量图像 pilot（MinAtar 类 + 单 seed + 判停，条款三：预算以第三方复现为锚）；**Phase 3 暂缓**（无裁决场）、**Pendulum 复议维持压轴**、**ROSMO 哨兵组件提案已否决**（叠加 29.6k vs 哨兵 ~8.7k）。Pendulum/Platform 已降级非关键路径 |
