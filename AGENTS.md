...
| **刚闭环** | **v0.23.1**：**MuZero canonical 完全体达标**——CartPole-v0 greedy eval **199.5 ≥195**（SAC ✅ / PPO ✅ / MuZero ✅ 三算法齐绿）。补齐 categorical value/reward（`support.rs`）+ latent min-max 归一化（`Var::amax/amin`）+ **absorbing state**（破 no-terminal 价值膨胀，平台期真因）+ canonical 梯度缩放（`Var::scale_gradient`，`DYNAMICS_GRADIENT_SCALE` 生效）+ `MuZeroConfig`（按环境配 sim）+ `reanalyze_game`（默认关）。这些是 `*Zero` 家族共享地基，已逐项验证；根因复盘见 [`.issue/_archive/muzero_cartpole_scalar_value_plateau.md`](.issue/_archive/muzero_cartpole_scalar_value_plateau.md) |
