...
- [ ] **⑭ PER 优先回放臂**（Simulus B1「loss 优先回放」转正；2026-07-05 晚实现
      ...
      底座 = ⑫/⑬ 赢家（载体默认 ⑫ 真规则底座，⑬ 胜出则公开修订），唯一新变量
      = `per=true`。判读（对照同底座）：naive0 中位抬升 ≥0.15 或 naive1 ≥0.3 =
      正信号；带内持平 = 排除（瓶颈不在消费端）；vs random <0.9 = 优先化过激
      记负不下钻。契约单测 5 项锁死（FIFO 镜像/比例采样/α=0 退化/零质量兜底）。
      **⏸ 挂起（2026-07-07 用户指示）**：⑬ 裁决后底座条款触发（税主导 → ⑭ 应在
      **⑬ learned dynamics 底座**测「通用组件能否收窄税差」——机制对症面：
      |ν−z| 最大的样本恰是「模型没看见终局要来」的局面，其 unroll 窗口正好给
      dynamics/reward 头补终局转移的监督）；用户暂缓启动，先考虑是否优先做
      seed 级并行提速（性能台账候选 #8 便宜档）。启动前按届时决定改载体底座
      （一行 `true_rules_tree` 开关）。
