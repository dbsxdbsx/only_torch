...
| P2 | 扩大 Segmentation Evolution benchmark | 未做 | U-Net-lite 已给出更强传统对照；需要确认 evolution 在同类 64x64 benchmark 上的成本、指标和结构族选择 | 先把 `evolution_overlapping_shapes_semantic_segmentation` 扩到更接近传统 benchmark 的样本量 / 图像尺度；沿用 Mean IoU、family counts、`backward_forward` / `backward_propagate` timing 记录 |
