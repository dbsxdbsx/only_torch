...
| P4 | Deformable Conv2d | 通用算子与传统基线已完成；evolution 最小接入已完成 | 适合在 segmentation 有基准后做，否则难判断收益；本轮先走“通用算子 → 传统手写示例 → evolution 接入”路线 | 已新增 `NodeTypeDescriptor::DeformableConv2d`、raw node 前反向、`DeformableConv2d` Layer、PyTorch / torchvision 数值对照单测、`deformable_conv2d_segmentation` 传统示例；segmentation InsertLayer 已有概率生成 DeformableConv2d block，并补构图测试。后续若继续做 FM edge 类型切换，再扩展 `ChangeFMEdgeType` |
